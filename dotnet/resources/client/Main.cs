...
namespace NeptuneEvo
{
    ...
    { 
        ...
        public void onResourceStart()
        {
            ...
            {
                // help on spawn
                NAPI.TextLabel.CreateTextLabel("~y~www.exilenetwork.de", new Vector3(-1028.4111, -2731.3354, 15.545905), 20f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                NAPI.TextLabel.CreateTextLabel("~g~Du hast eine Frage?\nstelle sie in /report", new Vector3(-1028.4111, -2731.3354, 14.545905), 20f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                NAPI.TextLabel.CreateTextLabel("~g~detaillierte Info - F10", new Vector3(-1028.4111, -2731.3354, 13.545905), 20f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                    ...
                    try
                    {
                            ...
                            player.HasData("CUFFED_BY_COP") && player.GetData<bool>("CUFFED_BY_COP") && Players[player].DemorganTime <= 0)
                        {
                            if (Players[player].WantedLVL == null)
                                Players[player].WantedLVL = new WantedLevel(3, "Сервер", new DateTime(), "Exit during detention");
        ...
        public void ClientEvent_SyncWP(Player player, float X, float Y)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You transferred data to your route!", 3000);
            ...
            try
            {
                ...
                switch (callback)
                {
                    ...
                    case "fuelcontrol_news":
                        ...
                        {
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Enter the correct data", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You installed the daily fuel limit in ${limit} for {fracName}", 3000);
                        return;
                    case "club_setprice":
                        try
                        {
                    ...
                    case "mayor_take":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Not enough funds in the treasury", 3000);
                            return;
                        }
                    ...
                    case "call_police":
                        if (text.Length == 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Enter the cause", 3000);
                            return;
                        }
                    ...
                    case "take_stock":
                        ...
                        if (Admin.IsServerStoping)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Server can not accept this actionе", 3000);
                            return;
                        }
                    ...
                    case "sellcar":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Such a car no longer exists", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered {target.Name} Buy your {vName} ({number}) for {price}$", 3000);

                        Trigger.ClientEvent(target, "openDialog", "BUY_CAR", $"{player.Name} Suggested to buy you {vName} ({number}) for ${price}");
                    ...
                    case "item_drop":
                        {
                            ...
                            if (Int32.TryParse(text, out int dropAmount))
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no so much {nInventory.ItemsNames[(int)item.Type]}", 3000);
                                    return;
                                }
                    ...
                    case "item_transfer_toveh":
                        {
                            ...
                            if (Int32.TryParse(text, out transferAmount))
                            {
                                ...
                                if (item.Type == ItemType.Material)
                                {
                                    ...
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"It is impossible to download such a number of mats", 3000);
                                        return;
                                    }
                                }
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "In the car is not enough", 3000);
                                    return;
                                }
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Increasing data", 3000);
                                return;
                            }
                        }
                        return;
                    case "item_transfer_tosafe":
                        {
                            ...
                            if (tryAdd == -1 || tryAdd > 0)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Not enough Places in Safe", 3000);
                                return;
                            }
                    ...
                    case "item_transfer_tofracstock":
                        {
                            ...
                            if (tryAdd == -1 || tryAdd > 0)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Not enough Places in warehouse", 3000);
                                return;
                            }
                    ...
                    case "item_transfer_toplayer":
                        {
                            ...
                            if (tryAdd == -1 || tryAdd > 0)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Player is not enough", 3000);
                                GUI.Dashboard.OpenOut(player, new List<nItem>(), changeTarget.Name, 5);
                                return;
                            }
                    ...
                    case "item_transfer_fromveh":
                        {
                            ...
                            if (Int32.TryParse(text, out transferAmount))
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no so much in the car {nInventory.ItemsNames[(int)item.Type]}", 3000);
                                    return;
                                }
                    ...
                    case "item_transfer_fromsafe":
                        {
                            ...
                            if (count < transferAmount)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no so much in the box {nInventory.ItemsNames[(int)item.Type]}", 3000);
                                return;
                            }
                            int tryAdd = nInventory.TryAdd(player, new nItem(item.Type, transferAmount));
                            if (tryAdd == -1 || tryAdd > 0)
                    ...
                    case "item_transfer_fromfracstock":
                        {
                            ...
                            if (count < transferAmount)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no so much in stock {nInventory.ItemsNames[(int)item.Type]}", 3000);
                                return;
                            }
                            int tryAdd = nInventory.TryAdd(player, new nItem(item.Type, transferAmount));
                            if (tryAdd == -1 || tryAdd > 0)
                    ...
                    case "smsadd":
                        {
                            ...
                            if (Int32.TryParse(text, out num))
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "You have a maximum number of contacts.", 3000);
                                    return;
                                }
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You added new contact {num}", 3000);
                            }
                            else
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Incorrect data", 3000);
                    ...
                    case "numcall":
                        {
                            ...
                            if (Int32.TryParse(text, out num))
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Player with this number not found", 3000);
                                    return;
                                }
                    ...
                    case "smssend":
                        {
                            ...
                            if (t == null)
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Player Offline", 3000);
                                return;
                            }
                            if (!MoneySystem.Bank.Change(Players[player].Bank, -10, false))
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Not enough funds on a bank account", 3000);
                                return;
                            }
                            ...
                            string notif = $"Message for {Players[player].Contacts[num]}: {text}";
                    ...
                    case "smsname":
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "The name contains a prohibited symbol..", 3000);
                                return;
                            }
                            int num = player.GetData<int>("SMSNUM");
                    ...
                    case "make_ad":
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You can not file an ad currently", 3000);
                                return;
                            }

                            if (Fractions.LSNews.AdvertNames.Contains(player.Name))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You already have one announcement in the queue", 3000);
                                return;
                            }

                            if (text.Length < 15)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Too short announcement", 3000);
                                return;
                            }
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You do not have enough cash in the bank", 3000);
                                return;
                            }
                    ...
                    case "player_ticketsum":
                        ...
                        Trigger.ClientEvent(player, "openInput", "Disposition penalty (reason)", "Reason", 50, "player_ticketreason");
                        break;
                    ...
                    case "enter_promocode":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have already activated promotional!", 3000);
                            return;
                        }

                        if (text.Contains(Players[player].GenPromo))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You cannot enter your promotion!", 3000);
                            return;
                        }
                        ...
                        if (Players[player].LVL < 3)
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Promocode {Main.Accounts[player].PromoCodes[0]} Successfully activated!Prize you get when the 3rd level is reached.", 5000);
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Promotkode can only be activated until 3rd level!", 3000);
                            return;
                        }
                        break;
                }
        ...
        public async void RestorePassword_event(Player client, byte state, string loginorcode)
        {
            try
            {
                ...
                { // Отправка кода
                    ...
                    {
                        Log.Debug($"Error trying to restore the password from the account!", nLog.Type.Warn);
                        return;
                    }
                    ...
                    {
                        Log.Debug($"SocialClub does not match SocialClub when registering", nLog.Type.Warn);
                        return;
                    }
                ...
                { // Ввод кода и проверка
                    if (Main.RestorePass.ContainsKey(client))
                    {
                        if (client.GetData<string>("RealSocialClub") == Main.RestorePass[client].Item3)
                        {
                            ...
                            {
                                Log.Debug($"{client.GetData<string>("RealSocialClub")} successfully restored the password!", nLog.Type.Info);
                                ...
                                Notify.Send(client, NotifyType.Success, NotifyPosition.Center, "Your password was reset, a new password must come in a message by mail, change it immediately after entering the command /password", 10000);
        ...
        public async Task SignInOnTimer(Player player, string login, string pass)
        {
            try
            {
                ...
                if (ban != null)
                {
                    ...
                    {
                        NAPI.Task.Run(() => Trigger.ClientEvent(player, "kick", $"You are blocked before {ban.Until.ToString()}. Reason: {ban.Reason} ({ban.ByAdmin})"));
                        return;
                    }
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Account already authorized.", 3000);
                }
                else if (result == LoginEvent.Refused)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The data is entered incorrectly", 3000);
                }
                if (result == LoginEvent.SclubError)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "SocialClub from which you are connected, does not coincide with the one that is tied to the account.", 3000);
                }
        ...
        public async Task ClientEvent_signup(Player player, params object[] arguments)
        {
            try
            {
                ...
                if (player.HasData("CheatTrigger"))
                {
                    ...
                    {
                        //Log.Write($"CheatKick: {((Cheat)cheatCode).ToString()} on {player.Name} ", nLog.Type.Warn);
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Emergency error!Try moving.", 10000);
                        player.Kick();
                        return;
                    }
                }
                ...
                if (result == RegisterEvent.Error)
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Emergency error!", 3000);
                else if (result == RegisterEvent.SocialReg)
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The game account has already been registered on this SocialClub!", 3000);
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Error filling the fields!", 3000);
        ...
        public void RemoteEvent_cancelPressed(Player player)
        {
            try
            {
                if (!Main.Players.ContainsKey(player) || !player.GetData<bool>("IS_REQUESTED")) return;
                player.SetData("IS_REQUESTED", false);
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Cancel", 3000);
            }
                    ...
                    switch (callback)
                    {
                        ...
                        case "BUY_CAR":
                            {
                                ...
                                if (PublicGarage.spawnedVehiclesNumber.Contains(number))
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The seller must parre the car before selling", 3000);
                                    break;
                                }
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought {vData.Model} ({number}) for {price}$ in {seller.Name}", 3000);
                                Notify.Send(seller, NotifyType.Success, NotifyPosition.BottomCenter, $"{player.Name} I bought from you {vData.Model} ({number}) for {price}$", 3000);
                                break;
                            }
                        case "INVITED":
                            {
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You entered to {Fractions.Manager.FractionNames[fracid]}", 3000);
                                try
                                {
                                    Notify.Send(player.GetData<Player>("SENDERFRAC"), NotifyType.Success, NotifyPosition.BottomCenter, $"{player.Name} accepted an invitation to join your fraction", 3000);
                                }
                                catch { }
                                return;
                            }
                        case "INVITEDTOFAMILY":
                            {
                                try
                                {
                                    ...
                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You entered to {Families.Family.GetFamilyName(player.GetData<Player>("SENDERFAMILY"))}", 3000);
                                }
                                catch (Exception e)
                                {
                                ...
                                try
                                {
                                    Notify.Send(player.GetData<Player>("SENDERFAMILY"), NotifyType.Success, NotifyPosition.BottomCenter, $"{player.Name} accepted an invitation to join your family", 3000);
                                }
                                catch { }
                                return;
                            }
                        ...
                        case "CAR_SELL_TOGOV":
                            if(player.HasData("CARSELLGOV")) {
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You sold {vData.Model} ({vnumber}) for {price}$", 3000);
        ...
        private static void playedMinutesTrigger()
        {
            try
            {
                if (!oldconfig.SCLog)
                {
                    ...
                    if(now.Hour == 4) {
                        if(now.Minute == 5) NAPI.Chat.SendChatMessageToAll("!{#DF5353}[AUTO RESTART] Dear players, at 04:20, there will be automatic server restart.");
                        else if(now.Minute == 10) NAPI.Chat.SendChatMessageToAll("!{#DF5353}[AUTO RESTART] Dear players, we remind you that at 04:20 there will be an automatic server restart.");
                        else if(now.Minute == 15) NAPI.Chat.SendChatMessageToAll("!{#DF5353}[AUTO RESTART] Dear players, we remind you that at 04:20 there will be an automatic server restart.");
                        else if(now.Minute == 20) {
                            NAPI.Chat.SendChatMessageToAll("!{#DF5353}[AUTO RESTART] Dear players, now there will be an automatic reboot server, the server will be available again about 2-5 minutes.");
                            Admin.stopServer("Automatic reset");
                    ...
                    try
                    {
                        ...
                        if (!Players[p].IsBonused)
                        {
                            ...
                            else
                            {
                                ...
                                Notify.Send(p, NotifyType.Info, NotifyPosition.BottomCenter, "You received 20 donat currency and gift, for 2 hours online today", 3000);
                                Players[p].LastBonus = 0;
                                Players[p].IsBonused = true;
                                Accounts[p].RedBucks += 20;
                                Trigger.ClientEvent(p, "updlastbonus", $"Next bonus can be obtained only tomorrow"); //todo bonus
                                return;
                            }
                            DateTime date = new DateTime((new DateTime().AddMinutes(oldconfig.LastBonusMin - Players[p].LastBonus)).Ticks);
                            var hour = date.Hour;
                            var min = date.Minute;
                            Trigger.ClientEvent(p, "updlastbonus", $"{hour}h. {min}m."); //todo bonus
                        }
                        #endregion
                    }
                ...
                try
                {
                    ...
                    if (DateTime.Now.Hour == 19) //каждый день в 19 часов вечера, контейнеры будут активированы. Можете поменять тут время или добавить ещё какой-то час. Или вообще сделать, чтобы каждый час они активировались.
                    {
                        try
                        {
                            ...
                            NAPI.Chat.SendChatMessageToAll("!{#fc4615} [Port]: !{#ffffff}" + "The state brought a new batch of containers!");
                        }
                        catch (Exception e)
                        {
                        ...
                        try
                        {
                            ...
                            if (Players[player].HotelID != -1)
                            {
                                ...
                                if (Players[player].HotelLeft <= 0)
                                {
                                    ...
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "You were evicted from the hotel for non-payment", 3000);
                                }
                            }

                            if (Players[player].LastHourMin < 15)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have to play at least 15 minutes to get Peides", 3000);
                                continue;
                            }
                            ...
                            switch (Fractions.Manager.FractionTypes[Players[player].FractionID])
                            {
                                ...
                                case 1:
                                    ...
                                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You received unemployment benefits {payment}$", 3000);
                                ...
                                case 2:
                                    ...
                                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You received Salary {payment}$", 3000);
                                    MoneySystem.Wallet.Change(player, payment);
                            ...
                            if (Players[player].EXP >= 3 + Players[player].LVL * 3)
                            {
                                ...
                                if (Players[player].LVL == 3 && Accounts[player].PromoCodes[0] != "noref" && PromoCodes.ContainsKey(Accounts[player].PromoCodes[0]))
                                {
                                    if (!Accounts[player].PresentGet)
                                    {
                                        ...
                                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Congratulations, you got a reward for achieving 3 levels in promotion {promo}!", 3000);
                                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You got: Gold VIP for 2 days and $ 5000!", 3000);
                                        ...
                                        try
                                        {
                                            ...
                                            foreach (Player pl in Players.Keys.ToList())
                                            {
                                                if (Players.ContainsKey(pl) && Players[pl].UUID == PromoCodes[promo].Item3)
                                                {
                                                    ...
                                                    Notify.Send(pl, NotifyType.Info, NotifyPosition.Bottom, $"You Received $ 2500 for the achievement of 3 levels by the player {player.Name}", 2000);
                                                    isGiven = true;
                                                    break;
                                                }
                                            }
                                    ...
                                    else Notify.Send(player, NotifyType.Error, NotifyPosition.Bottom, "This account has already received a gift for activating promotion", 5000);
                                }
                            }
                            ...
                            if (Accounts[player].VipLvl > 0 && Accounts[player].VipDate <= DateTime.Now)
                            {
                                ...
                                Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, "VIP status with you", 3000);
                            }
                    ...
                    foreach (Business biz in BusinessManager.BizList.Values)
                    {
                        try
                        {
                            ...
                            if (PlayerNames.ContainsValue(owner))
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"The state has selected your business for non-payment of taxes", 3000);
                    ...
                    foreach (Houses.House h in Houses.HouseManager.Houses)
                    {
                        try
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "You have selected a house for non-payment of taxes.", 3000);
        ...
        private static void callback_smsdata(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                ...
                case "call":
                    ...
                    break;
                case "rename":
                    Trigger.ClientEvent(player, "openInput", "Renamation", $"Enter a new name for {num}", 18, "smsname");
                    break;
                case "remove":
        ...
        public static void CMD_BUILD(Player client)
        {
            try
            {
                client.SendChatMessage($"Willkommen auf {{#00FFFF}}{Full}!{{#FFF}} THE EXILED Roleplay.{{#f39c12}}{StartDate}");
            }
            catch { }
        }
        ...
        public static async Task OpenPlayerMenu(Player player)
        {
            ...
            if (oldconfig.VoIPEnabled)
            {
                ...
                if (vpmd.Target != null)
                {
                    ...
                    string text = (vpmd.CallingState == "callMe") ? "Reject call" : (vpmd.CallingState == "callTo") ? "Cancel call" : "Complete Call";
        ...
        private static void callback_mainmenu(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                ...
                case "contacts":
                    ...
                    return;
                case "ad":
                    Trigger.ClientEvent(player, "openInput", "Ad", "6$ For every 20 characters", 100, "make_ad");
                    return;
                case "openhouse":
                    {
                        Houses.House house = Houses.HouseManager.GetHouse(player);
                ...
                case "leavehouse":
                    {
                        ...
                        if (house == null)
                        {
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You do not live in the house", 3000);
                            MenuManager.Close(player);
                            return;
                        }
                    ...
                    return;
                case "promo":
                    Trigger.ClientEvent(player, "openInput", "Promocode", "Enter promotion", 10, "enter_promocode");
                    return;
                case "acceptcall":
        ...
        private static Dictionary<string, Vector3> Points = new Dictionary<string, Vector3>()
        {
            ...
            //{ "Hospital", new Vector3(240.7599, -1379.576, 32.74176) },
            ...
            { "Taxpark", new Vector3(903.3215, -191.7, 73.40494) },
            { "Bus depot", new Vector3(462.6476, -605.5295, 27.49518) },
            { "Parking lawn mowers", new Vector3(-1331.475, 53.58579, 53.53268) },
            { "Parking truckers", new Vector3(588.2037, -3037.641, 6.303829) },
            { "Parking collectors", new Vector3(915.9069, -1265.255, 25.52912) },
            { "Parking auto mechanics", new Vector3(473.9508, -1275.597, 29.60513) },
            { "Marabunda", new Vector3(857.0747,-2207.008,0) },
            ...
            { "Arena (Gun Game)", GanGame.gangameCheckpoints[0] },
        ...
        private static void callback_gps(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                ...
                case "Taxpark":
                case "Bus depot":
                case "Parking lawn mowers":
                case "Parking truckers":
                case "Parking collectors":
                case "Parking auto mechanics":
                case "Marabunda":
                case "Vagos":
                case "Ballas":
                case "Фемелис":
        ...
        private static void callback_services(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                ...
                case "sheriff":
                    MenuManager.Close(player);
                    Trigger.ClientEvent(player, "openInput", "Call sheriff", "What happened?", 30, "call_sheriff");
                    return;
                case "ems":
                    MenuManager.Close(player);
        ...
        public static void OpenMayorMenu(Player player)
        {
            ...
            menuItem.Text = $"Collected for the last hour: {Fractions.Cityhall.lastHourTax}$";
            menu.Add(menuItem);
        ...
        private static void callback_mayormenu(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "take":
                    MenuManager.Close(player);
                    Trigger.ClientEvent(player, "openInput", "Get money out of the treasury", "number", 6, "mayor_take");
                    return;
                case "put":
                    MenuManager.Close(player);
        ...
        public static void OpenFuelcontrolMenu(Player player)
        {
            ...
            menuItem.Text = $"Police. Left today: {Fractions.Stocks.fracStocks[18].FuelLeft}/{Fractions.Stocks.fracStocks[18].FuelLimit}$";
            menu.Add(menuItem);
        ...
        private static void callback_fuelcontrol(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                case "set_city":
                    ...
                    return;
                case "set_police":
                    Trigger.ClientEvent(player, "openInput", "Set limit", "Enter the fuel limit Police in dollars", 5, "fuelcontrol_police");
                    return;
                case "set_sheriff":
                    Trigger.ClientEvent(player, "openInput", "Set limit", "Enter the fuel limit Sheriff in dollars", 5, "fuelcontrol_sheriff");
                    return;
                case "set_ems":
                    Trigger.ClientEvent(player, "openInput", "Set limit", "Enter the fuel limit For EMS. in dollars", 5, "fuelcontrol_ems");
                    return;
                case "set_fib":
                    Trigger.ClientEvent(player, "openInput", "Set limit", "Enter the fuel limit For FIB in dollars", 5, "fuelcontrol_fib");
                    return;
                case "set_army":
    ...
    public static class PasswordRestore
    {
        ...
        private static string mailFrom = config.TryGet<string>("From", "admin@exilenetwork.de");
        ...
        private static string mailBody2 = config.TryGet<string>("Body2", "<p>You have successfully restored the password, your new password: {0}</p>");
        ...
        public static void SendEmail(byte type, string email, int textcode)
        {
            try
            {
                ...
                smtpClient.Send(msg);
                if (type == 0) Log.Debug($"The message with the password recovery code has been successfully sent to {email}!", nLog.Type.Success);
                else Log.Debug($"The message with the new password has been successfully sent to {email}!", nLog.Type.Success);
            }
            catch (Exception ex)
            {
