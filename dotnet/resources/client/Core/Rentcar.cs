...
namespace NeptuneEvo.Core
{
    class Rentcar : Script
    {
        ...
        {
            new Tuple<string, Vector3>("Auto", new Vector3(-526.91406, 61.327232, 51.459885)),
            new Tuple<string, Vector3>("Scooters", new Vector3(285.24445, -349.28256, 43.871384)),
            new Tuple<string, Vector3>("Boats", new Vector3(-853.62213, -1327.3962, 0.9336101)),
            
            // Newbie respawn
            new Tuple<string, Vector3>("Auto", new Vector3(-989.4564, -2696.0186, 12.71069)),
            new Tuple<string, Vector3>("Scooters", new Vector3(-1029.6763, -2672.7966, 12.7107525)),

            new Tuple<string, Vector3>("Auto", new Vector3(2493.2422, 4115.2075, 37.17832)),
            new Tuple<string, Vector3>("Scooters", new Vector3(-269.83328, 6059.1196, 30.344475)),
        ...
        public void onResourceStart()
        {
            try
            {
                foreach (var v in RentAreasTypes)
                {
                    ...
                    switch (v.Item1)
                    {
                        case "Auto": blipIcon = 225;
                            break;
                        case "Boats": blipIcon = 404;
                            break;
                        case "Scooters": blipIcon = 661;
                            break;
                    ...
                    NAPI.Blip.CreateBlip(blipIcon, v.Item2, 0.8f, 1, Main.StringToU16($"Transport rental ({v.Item1})"), 255, 0, true, 0, 0);
                }
            }
        ...
        public static void RentNPC_MenuSelectVehicle(Player player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must not be in the car!", 3000);
                    return;
                }

                if (player.HasData("RENTED_CAR"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have already paid for the rental of another vehicle!", 3000);
                    return;
                }
        ...
        private static void callback_npcvehmenu(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                ...
                case "faggio3":
                    ...
                    Trigger.ClientEvent(client, "openDialog", "RENT_CAR_NPC", $"Do you want to rent a vehicle - {item.ID} for price $100?");
                    break;
                case "close":
        ...
        public void Event_OnPlayerEnterVehicle(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This transport is already rented by another player.!", 3000);
                ...
                if (vehicle.GetData<Player>("DRIVER") == null)
                {
                    if (player.HasData("RENTED_CAR"))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have already paid for renting another transport!", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This transport is intended only for beginners!", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You do not have a license for water transport to conclude a lease agreement!", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                        return;
                    }
                    //
                    ...
                    Trigger.ClientEvent(player, "openDialog", "RENT_CAR", $"You want to rent this transport for ${price}?");
                }
                else
                {
        ...
        public void Event_OnPlayerExitVehicle(Player player, Vehicle vehicle)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"Through 3 minute Transport rental will end if you do not sit in сar again.", 3000);
        ...
        private void timer_playerExitRentVehicle(Player player, Vehicle vehicle)
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"The lease term ended.Transport was towed to the parking lot.", 3000);
                        RespawnCar(vehicle);
        ...
        public static void RentCar(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough Funds for rent.", 3000);
                VehicleManager.WarpPlayerOutOfVehicle(player);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You have successfully rented a transport (the rental of transport will end if you do not get into the vehicle again within 3 minutes.)", 3000);
        }
