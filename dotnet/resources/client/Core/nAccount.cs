...
namespace NeptuneEvo.Core.nAccount
{
    public class Account : AccountData
    {
        ...
        public void LoadSlots(Player player)
        {
                    ...
                    if (uuid > -1)
                    {
                        ...
                        else
                        {
                            if(Main.PlayerNames.ContainsKey(uuid) && Main.PlayerSlotsInfo.ContainsKey(uuid)) {
                                ...
                                Notify.Send(player, NotifyType.Error, NotifyPosition.Center, $"Unfortunately, it is impossible to obtain data about the character with a passport number {uuid}, Contact Team on the forum.", 5000);
                                return;
                            }
                        }
                        data.Add(subData);
            ...
            {
                if (Main.LoggedIn.ContainsKey(Login)) Main.LoggedIn.Remove(Login);
                Notify.Send(player, NotifyType.Error, NotifyPosition.Center, "Unfortunately, it is impossible to obtain data on account characters, inform the forum in Tempe.", 5000);
        ...
        public async Task DeleteCharacter(Player player, int slot, string firstName_, string lastName_, string password_)
        {
            ...
            if (ban != null && ban.CheckDate()) {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "It is impossible to remove the character that is in the bath.", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The name and surname do not correspond to the character on the selected slot.", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Wrong password from account", 3000);
                return;
            }

            foreach (var b in biz)
                BusinessManager.changeOwner($"{firstName}_{lastName}", "State");
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"The character {firstName} {lastName} successfully removed", 3000);
