        ...
        public static void onUse(Player player, nItem item, int index)
        {
            try
            {
                ...
                if (nInventory.ClothesItems.Contains(item.Type) && item.Type != ItemType.BodyArmor && item.Type != ItemType.Mask)
                {
                    ...
                    {
                        var error_gender = (clothesGender) ? "Male" : "Women's";
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You can not use it now", 3000);
                        GUI.Dashboard.Close(player);
                        return;
                    }
                }
                ...
                if (nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type))
                {
                    ...
                    else
                    {
                        ...
                        Commands.RPChat("me", player, $"pulled out {nInventory.ItemsNames[(int)item.Type]}");
                        item.IsActive = true;
                ...
                if (nInventory.AlcoItems.Contains(item.Type))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are drunk to such an extent that you can not open a bottle", 3000);
                        return;
                    }
                    ...
                    Commands.RPChat("me", player, "He drank a bottle " + nInventory.ItemsNames[(int)item.Type]);
                ...
                switch (item.Type)
                {
                    ...
                    case ItemType.Glasses:
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You can not wear these glasses with a mask", 3000);
                                    return;
                                }
                    ...
                    case ItemType.Hat:
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You can not wear this headdress with the mask", 3000);
                                    return;
                                }
                                var itemData = (string)item.Data;
                                var variation = Convert.ToInt32(itemData.Split('_')[0]);
                    ...
                    case ItemType.Accessories:
                        {
                            ...
                            else
                            {
                                ...
                                else if (Customization.AccessoryRHand[gender].ContainsKey(variation))
                                {
                                    ...
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Busy both hands", 3000);
                                        return;
                                    }
                                }
                                else 
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The left hand is busy, and on the right no one clocks", 3000);
                                    return;
                                }
                            }
                            return;
                    ...
                    case ItemType.Undershit:
                        {
                            ...
                            else
                            {
                                if (Customization.CustomPlayerData[Main.Players[player].UUID].Clothes.Top.Variation == Customization.EmtptySlots[gender][11])
                                {
                                    if (underwear.Top == -1)
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This clothing can only be apparent under the bottom", 3000);
                                        return;
                                    }
                                ...
                                else
                                {
                                    ...
                                    if (nowTop != null)
                                    {
                                        ...
                                        {
                                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This clothing is incompatible with your outerwear.", 3000);
                                            return;
                                        }
                    ...
                    case ItemType.Drugs:
                        if (!player.HasData("USE_DRUGS") || DateTime.Now > player.GetData<DateTime>("USE_DRUGS"))
                        {
                            ...
                            Commands.RPChat("me", player, $"Clamp boiled");
                    ...
                    case ItemType.GasCan:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"In the car full tank", 3000);
                            GUI.Dashboard.Close(player);
                            return;
                        }
                        fuel += 30;
                    ...
                    case ItemType.Lockpick:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"It is impossible to use at the moment", 3000);
                            GUI.Dashboard.Close(player);
                            return;
                        }
                        ...
                        Trigger.ClientEvent(player, "showLoader", "There is a hacking", 1);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You started hacking the door", 3000);
                        break;
                    case ItemType.ArmyLockpick:
                        if (!player.IsInVehicle || player.Vehicle.DisplayName != "BARRACKS")
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be in the Military Carriage of Materials", 3000);
                            return;
                        }
                        if (VehicleStreaming.GetEngineState(player.Vehicle))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The car is already headed", 3000);
                            return;
                        }
                        var lucky = new Random().Next(0, 5);
                        Log.Debug(lucky.ToString());
                        if (lucky == 5)
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not work out the car.Try again", 3000);
                        else
                        {
                            VehicleStreaming.SetEngineState(player.Vehicle, true);
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You managed to start the car", 3000);
                        }
                        break;
                    case ItemType.Present:
                        player.Health = (player.Health + 10 > 100) ? 100 : player.Health + 10;
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You opened a gift, in it were:", 3000);
                        ...
                        Commands.RPChat("me", player, $"Opened a gift {types.Item1} + {types.Item2}");
                        break;
                    case ItemType.Payek:
                        if (player.VehicleSeat == 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can not leave dry. Pyatok while driving.", 3000);
                            return;
                        }
                        if (!player.HasData("USE_MEDKIT") || DateTime.Now > player.GetData<DateTime>("USE_MEDKIT"))
                        {
                            ...
                            Commands.RPChat("me", player, $"Used food");
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Try to use later", 3000);
                    ...
                    case ItemType.LSPDDrone:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not an employee!", 3000);
                            return;
                        }
                        if (!NAPI.Data.GetEntityData(player, "ON_DUTY"))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must start the work day!", 3000);
                            return;
                        }
                    ...
                    case ItemType.CasinoChips:
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The chips are used to play in the casino (GPS - Entertainment - Diamond Casino).", 3000);
                        break;
                }
                nInventory.Remove(player, item.Type, 1);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You used {nInventory.ItemsNames[item.ID]}", 3000);
                GameLog.Items($"player({Main.Players[player].UUID})", "use", Convert.ToInt32(item.Type), 1, $"{item.Data}");
                GUI.Dashboard.Close(player);
            }
            catch (Exception e)
        ...
        public static void onDrop(Player player, nItem item, dynamic data)
        {
            try
            {
                ...
                if (data != null && (int)data != 1)
                    Commands.RPChat("me", player, $"Threw {nInventory.ItemsNames[(int)item.Type]}");
