...
namespace NeptuneEvo.Core
{
    class RodManager : Script
    {
        ...
        public static void allowfish(Player player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Something pecked", 1000);
        ...
        public static void giveRandomFish(Player player)
        {
            ...
            if (player.GetData<int>("FISHLEVEL") == 1)
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You caught fish {GetNameByItemType(RodManager.FishItems1[fishco])}", 3000);
            }
            if (player.GetData<int>("FISHLEVEL") == 2)
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You caught fish {GetNameByItemType(RodManager.FishItems2[fishco])}", 3000);
            }
            if (player.GetData<int>("FISHLEVEL") == 3)
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You caught fish {GetNameByItemType(RodManager.FishItems3[fishco])}", 3000);
            }
            RodManager.crashpros(player);
        }
        ...
        {
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Fish has gone from the hook!", 3000);
            RodManager.crashpros(player);
        }
        ...
        public static void useInventory(Player player, int level)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have to be in the car!", 3000);
            ...
            if (aItem == null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You do not have bait", 3000);
                return;
            }
            if (player.GetData<bool>("ALLOWFISHING") == false || player.GetData<bool>("ALLOWFISHING") == false)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "In this place can not be fisted", 3000);
                return;
            }
            var rndf = new Random();
        ...
        public class Roding
        {
            ...
            public Roding(int id, Vector3 areapoint, float radius)
            {
                ...
                // Create blip
                blip = NAPI.Blip.CreateBlip(68, AreaPoint, 1, 67, "Place for fishing", 255, 0, true);
