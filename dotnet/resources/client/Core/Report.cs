...
namespace NeptuneEvo.Core
{
    class ReportSys : Script
    {
        ...
        public void ReportSend(Player player, int ID, string answer)
        {
            ...
            {
                player.SendChatMessage("This complaint is more not available to change.");
                Remove(ID, player);
            }
        }
        #endregion
        ...
        public static void AddReport(Player player, string question)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You sent a question: {question}", 3000);
                ...
                foreach (Player p in Main.Players.Keys.ToList())
                {
                    ...
                    if (Main.Players[p].AdminLVL >= 1) p.SendChatMessage($"~r~Report from {player.Name.Replace('_', ' ')} (ID {player.Value}): {question}");
                }
            }
            catch (Exception e)
            {
        ...
        private static void AddAnswer(Player player, int repID, string response)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        target.SendChatMessage($"~r~Answer from {player.Name} ({player.Value}): {response}");
