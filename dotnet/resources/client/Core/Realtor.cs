    ...
    class Realtor : Script
    {
        ...
        public static void EnterShapeRealtor()
        {
            try
            {
                ...
                blip = NAPI.Blip.CreateBlip(374, PositionRealtor, 0.9f, 2, "Real estate agency", shortRange: true, dimension: 0);
        ...
        public static void BuyInfoHome(Player player, int hclass, float x, float y)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough money to buy information", 3000);
            }
            else
            {
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Route installed", 3000);
