                ...
                if (id >= 0 && id < NAPI.Server.GetMaxPlayers())
                {
                    ...
                    if (target != null)
                    {
                        if (target != player)
                        {
                            ...
                            else player.SendChatMessage("The player for data ID has not yet logged in.");
                        }
                    }
                    else player.SendChatMessage("Player under ID. " + id + " absent.");
                }
                else player.SendChatMessage("The player ID is invalid (less than 0 or more number of slots).");
            }
        }
            ...
            if (Main.Players.ContainsKey(player))
            {
                if (player.GetData<bool>("spmode") == true)
                {
                    ...
                    Timers.StartOnce(400, () => {
                        ...
                        player.SendChatMessage("You left the observer mode.");
                    });
                }
                else player.SendChatMessage("You are not in the Observer mode.");
            }
        }
    }
}
