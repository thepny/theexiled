...
namespace NeptuneEvo.Core
{
    public class SafeMain : Script
    {
        ...
        public static void startSafeDoorCracking(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no drill to hack", 3000);
                return;
            }
            if (isCracking)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Hacking already started", 3000);
                return;
            }
            if (isOpen)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The door to the storage is already open.", 3000);
                return;
            }
            ...
            canBeClosed = false;
            Manager.sendFractionMessage(6, "Someone is trying to hack the door to the City Hall.");
            Manager.sendFractionMessage(7, "Someone is trying to hack the door to the City Hall.");
            Manager.sendFractionMessage(9, "Someone is trying to hack the door to the City Hall.");
            Manager.sendFractionMessage(14, "Someone is trying to hack the door to the City Hall.");
        }
        ...
        public void onPlayerEnterVehicle(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                if ((player.HasData("HAND_MONEY") || player.HasData("HEIST_DRILL")) && player.VehicleSeat == 0 && vehicle.Class != 8)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can not sit in the car with bags", 3000);
                }
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (stage == 2)
                    {
                        ...
                        //NAPI.Player.PlaySoundFrontEnd(player, "Drill_Pin_Break", "DLC_HEIST_FLEECA_SOUNDSET");
                        Trigger.ClientEvent(player, "dial", "close");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You successfully hacked a safe", 2000);
                    }
                    else
                    {
                        stage++;
        ...
        public static void interactSafe(Player player)
        {
            ...
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Hacking only in mask", 3000);
                    return;
                }

                if (safe.Occupier != null && NAPI.Player.GetPlayerFromHandle(safe.Occupier) != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This safe is already hacked", 3000);
                    return;
                }
                if (Fractions.Manager.FractionTypes[Main.Players[player].FractionID] != 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Available only for gangs", 3000);
                    return;
                }
                ...
                if (count == 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no laundry", 3000);
                    return;
                }
                if (DateTime.Now < NextRobbery && NowRobberyID != safe.ID)
                {
                    DateTime g = new DateTime((NextRobbery - DateTime.Now).Ticks);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Try around {g.Minute}:{g.Second}", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"With you should be at least one more Tru Gangster", 3000);
                    //return;
                }
                ...
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"A minute later cops will arrive here, it will be attentive!", 3000);
                Manager.sendFractionMessage(7, $"Safe by address {safe.Address} Trying to hack");
                Manager.sendFractionMessage(9, $"Safe by address {safe.Address} Trying to hack");
                ...
                {
                    var wantedLevel = new WantedLevel(4, "Police", DateTime.Now, "Safety robbery");
        ...
        public static void dropMoneyBag(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can not do it while in the house / garage", 3000);
                return;
            }
        ...
        public static void MoneyFlow(Player player)
        {
            ...
            {
                NAPI.Chat.SendChatMessageToPlayer(player, "~b~[Сергей Мавроди] ~w~What have you forgotten here?");
                return;
            }
            var all_money = 0;
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You wash {(int)(all_money * 0.97)}$. Mavrodi taken {(int)(all_money * 0.03)}$ For their services", 3000);
        }
        ...
        public static void CMD_CreateSafe(Player player, int id, float distance, int minamount, int maxamount, string address)
        {
            ...
            if (safe != null)
            {
                NAPI.Chat.SendChatMessageToPlayer(player, "~r~[Error] ~w~Safe with such ID already exists.");
                return;
            }
        ...
        public static void CMD_RemoveSafe(Player player)
        {
            ...
            {
                player.SendChatMessage("~r~[Error] ~w~You must be near the safe.");
                return;
            }
            ...
            try
            {
                switch (index)
                {
                    ...
                    case 1:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a bag with a drill for robbery", 3000);
                        return;
                    case 2:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a lock for locks", 3000);
                        return;
                    case 3:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a military launder", 3000);
                        return;
                    case 4:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought hands for hands", 3000);
                        return;
                    case 5:
                        if (Main.Players[player].Money < 600)
                        {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a bag on the head", 3000);
                        return;
                    case 6:
                    ...
                    case 7:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Hmm ... you have not enough funds", 3000);
                            return;
                        }
                        var aItem = nInventory.Find(Main.Players[player].UUID, ItemType.BodyArmor);
                        if (aItem != null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Hmm ... you already have a body armor", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Hmm ... you got a body armor", 3000);
                        return;
                }
            }
            ...
            switch (item.ID)
            {
                case "change":
                    ...
                    if (Main.Players[player].FractionID == 6 && Main.Players[player].FractionLVL >= 14)
                    {
                        if (isCracking)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"It is impossible to do it now", 3000);
                            return;
                        }
                        if (!canBeClosed)
                        {
                    ...
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can not do it", 3000);
                    return;
                case "crack":
                    MenuManager.Close(player);
    ...
    public class Safe
    {
        ...
        public void Loot(Player player)
        {
            ...
            if (SafeLoot == 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no money in the safe anymore", 3000);
                return;
            }
            ...
            if (player.HasData("HAND_MONEY"))
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Your bag is completely clogged with money", 3000);
                    return;
                }
            ...
            else
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You took a bag with {money}$", 3000);
            }
