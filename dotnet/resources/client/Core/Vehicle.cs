...
namespace NeptuneEvo.Core
{
    class VehicleManager : Script
    {
                    ...
                    if (NAPI.Data.GetEntityData(vehicle, "ACCESS") == "FRACTION")
                    {
                        if (NAPI.Data.GetEntityData(vehicle, "FRACTION") == 14 && vehicle.DisplayName == "BARRACKS")
                        {
                            ...
                            if ((fracid >= 1 && fracid <= 5) || (fracid >= 10 && fracid <= 13))
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Illegal organizations can not sit in the car from 00:00 to 10:00", 3000);
                                    return;
                                }
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"To start the engine, use military launder", 3000);
                                return;
                            }
                            else if (fracid == 14)
                            {
                                ...
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"To start the engine, press B", 3000);
                                return;
                            }
                            else
                            {
        ...
        private static void callback_sellcar(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            Trigger.ClientEvent(player, "openInput", "Sell a car", "Введите цену", 7, "sellcar");
        }
        #endregion
            ...
            switch (NAPI.Data.GetEntityData(vehicle, "ACCESS"))
            {
                case "HOTEL":
                    ...
                    if (Core.VehicleStreaming.GetLockState(vehicle))
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You opened the doors of the car", 3000);
                    }
                    else
                    {
                        Core.VehicleStreaming.SetLockStatus(vehicle, true);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You closed the doors of the car", 3000);
                    }
                    break;
                case "RENT":
        ...
        public static void onClientEvent(Player sender, string eventName, params object[] args)
        {
            switch (eventName)
            {
                ...
                case "engineCarPressed":
                    ...
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be in a driver's seat", 3000);
                        return;
                    }
                    ...
                    if (fuel <= 0)
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"The fuel tank is empty, it is impossible to start the car", 3000);
                        return;
                    }
                    switch (NAPI.Data.GetEntityData(vehicle, "ACCESS"))
                    {
                        case "HOTEL":
                            ...
                            {
                                Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no keys from this transport", 3000);
                                return;
                            }
                            if (Core.VehicleStreaming.GetEngineState(vehicle))
                            {
                                Core.VehicleStreaming.SetEngineState(vehicle, false);
                                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"You muffled the engine", 3000);
                            }
                            else
                            {
                        ...
                        case "beltCarPressed":
                            ...
                            if (!beltstate) Commands.RPChat("me", sender, "Fonded a seat belt");
                            else Commands.RPChat("me", sender, "He left the seat belt");
                    ...
                    if (Core.VehicleStreaming.GetEngineState(vehicle)) Commands.RPChat("me", sender, "Went a vehicle");
                    else Commands.RPChat("me", sender, "Pushed the vehicle");
                    return;
                ...
                switch (access)
                {
                    ...
                    case "WORK":
                        ...
                        if (player != null)
                        {
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Your working transport was destroyed. " + paymentMsg, 3000);
    ...
    class VehicleInventory : Script
    {
        public static void Add(Vehicle vehicle, nItem item)
        {
            ...
            foreach (Player p in Main.Players.Keys.ToList())
            {
                ...
                if (p.HasData("OPENOUT_TYPE") && p.GetData<int>("OPENOUT_TYPE") == 2 && p.HasData("SELECTEDVEH") && p.GetData<Vehicle>("SELECTEDVEH") == vehicle) GUI.Dashboard.OpenOut(p, vehicle.GetData<List<nItem>>("ITEMS"), "Trunk", 2);
            }
        }
