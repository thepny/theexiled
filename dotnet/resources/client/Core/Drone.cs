...
namespace NeptuneEvo.Core
{
    class Drone : Script
    {
        ...
        public void startSpecMissionLspd(Player client)
        {
            ...
            {
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not an employee!", 3000);
                return;
            }
            if (!NAPI.Data.GetEntityData(client, "ON_DUTY"))
            {
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You must start the work day!", 3000);
                return;
            }
