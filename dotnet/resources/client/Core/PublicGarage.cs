                        ...
                        if (c.Vehicle.GetData<Player>("OWNER") == c)
                        {
                            ...
                            else
                            {
                                Notify.Send(c, NotifyType.Error, NotifyPosition.BottomCenter,
                                    $"You have a house.go and pour in your garage!",
                                3000);
                            }
                        }
                        else
        ...
        public static void SpawnVehicle(Player c, string vNumber)
        {
            try
            {
                ...
                if (house == null || house.GarageID == 0)
                {
                    ...
                    {
                        Log.Write("Someone tried to create not his vehicle!");
                    ...
                    if (! access)
                    {
                        Notify.Send(c, NotifyType.Info, NotifyPosition.BottomCenter,
                            $"It seems you lost the keys to the car.Take a new one!",
                        3000);
            ...
            } catch(Exception e) { Log.Write("Error when creating a car from a public garage " + e.Message); }
