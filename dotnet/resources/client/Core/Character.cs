    ...
    public class Character : CharacterData
    {
            ...
            try
            {
                NAPI.Task.Run(() =>
                {
                    try
                    {
                        ...
                        if (IsBonused)
                        {
                            Trigger.ClientEvent(player, "updlastbonus", $"Next bonus can be obtained only tomorrow"); //todo bonus
                        }
                        else
                        {
                            ...
                            var hour = date.Hour;
                            var min = date.Minute;
                            Trigger.ClientEvent(player, "updlastbonus", $"{hour}h. {min}m."); //todo bonus
                        }
                        #endregion
                ...
                if (Warns > 0 && DateTime.Now > Unwarn)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"One warning was removed.You have left {Warns}", 3000);
                }
        ...
        public async Task<int> Create(Player player, string firstName, string lastName)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Error in the length of the name / surname", 3000);
                    return -1;
                }
