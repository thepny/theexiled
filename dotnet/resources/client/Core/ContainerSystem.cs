...
namespace NeptuneEvo.Core
{
    class ContainerSystem : Script
    {
        ...
        public static void OnResourceStart()
        {
            try
            {
                Blip portblip = NAPI.Blip.CreateBlip(50, new Vector3(1210, -2987, 0), 0.91f, 6, Main.StringToU16("Container system"), 254, 0, true, 0, 0);
                ...
                Log.Write($"Loaded {containers.Count} Containers");
            }
            catch (Exception e)
            {
        ...
        public void ChangeStateContainers(Player player, bool state)
        {
            ...
            if (state)
                NAPI.Chat.SendChatMessageToAll("!{#fc4122} [Port]: !{#ffffff}" + "The state brought a new batch of containers!");
        }
        #endregion
        ...
        public static void OpenContainer(Player player)
        {
            try
            {
                ...
                if (container.Donate)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Insufficient funds", 2500);
                        return;
                    }
    ...
    public class Container
    {
        ...
        public Container(int id, string name, Vector3 pos, Vector3 rot, int price, bool donate = false, string model = "prop_container_02a", string door_l = "prop_cntrdoor_ld_l", string door_r = "prop_cntrdoor_ld_r", string fence = "prop_fncsec_01b")
        {
            ...
            label = NAPI.TextLabel.CreateTextLabel("Inactive", pos + new Vector3(-2, 6.7, 1), 10f, 0.2f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
        ...
        public void Visible(bool state)
        {
            if (state)
            {
                label.Text = $"ID: {ID} \n~r~{Name} \n~s~Price: ~g~~h~{Price}{(Donate ? "Coins" : "$")}";
        ...
        public void GenerateLoot(Player player)
        {
            ...
            if (house == null || house.GarageID == 0)
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Your prize - {vName}", 2500);
            else
            {
                ...
                if (vNumber != "no")
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Your prize - {vName} will be delivered to the garage", 2500);
                }
            }
