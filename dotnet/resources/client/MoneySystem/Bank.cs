...
namespace NeptuneEvo.MoneySystem
{
    class Bank : Script
    {
        ...
        public static bool Transfer(int firstAccID, int lastAccID, long amount)
        {
            ...
            {
                if (firstAcc.Type == 1)
                    BankNotify(NAPI.Player.GetPlayerFromName(firstAcc.Holder), BankNotifyType.InputError, "This account does not exist!");
            ...
            {
                if (firstAcc.Type == 1)
                    BankNotify(NAPI.Player.GetPlayerFromName(firstAcc.Holder), BankNotifyType.PayError, "Not enough средств!");
        ...
        public static void BankNotify(Player player, BankNotifyType type, string info)
        {
            switch (type)
            {
                ...
                case BankNotifyType.PayError:
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Error writing off means", 3000);
                    return;
    ...
    class ATM : Script
    {
        ...
        public static void OpenATM(Player player)
        {
            ...
            if (acc.Bank == 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Register an account in the nearest bank branch", 3000);
                return;
            }
        ...
        public static void ClientEvent_ATMVAL(Player player, params object[] args)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Server now can not accept this action", 3000);
                    return;
                }
                var acc = Main.Players[player];
                ...
                switch (type)
                {
                    ...
                    case 2:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "It is impossible to translate so many funds at home.", 3000);
                            return;
                        }
                        if (!Bank.Change(acc.Bank, -Math.Abs(amount)))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough Tools.", 3000);
                            return;
                        }
                        ...
                        Trigger.ClientEvent(player,
                                "atmOpen", $"[2,'{Bank.Accounts[house.BankID].Balance}/{Convert.ToInt32(house.Price / 100 * 0.013) * 24 * 7}$','The amount of cash deposit']");
                        break;
                    case 3:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "It is impossible to translate so many funds to the business account..", 3000);
                            return;
                        }
                        if (!Bank.Change(acc.Bank, -Math.Abs(amount)))
                        {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Successful translation.", 3000);
                        Trigger.ClientEvent(player, "atmOpen", $"[2,'{Bank.Accounts[biz.BankID].Balance}/{Convert.ToInt32(biz.SellPrice / 100 * 0.013) * 24 * 7}$','Amount enroll']");
                        break;
                    case 4:
                        if (!Bank.Accounts.ContainsKey(amount) || amount <= 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The score is not found!", 3000);
                            Trigger.ClientEvent(player, "closeatm");
                            return;
                        }
                        NAPI.Data.SetEntityData(player, "ATM2ACC", amount);
                        Trigger.ClientEvent(player,
                                "atmOpen", "[2,0,'Amount for transfer']");
                    ...
                    case 44:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The next transaction will be possible for minute", 3000);
                            return;
                        }
                        int bank = NAPI.Data.GetEntityData(player, "ATM2ACC");
                        if (!Bank.Accounts.ContainsKey(bank) || bank <= 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The bill is not found!", 3000);
                            Trigger.ClientEvent(player, "closeatm");
                            return;
                        }
                ...
                switch (type)
                {
                    case 1:
                        switch (index)
                        {
                            case 0:
                                Trigger.ClientEvent(player,
                                    "atmOpen", "[2,0,'The amount of cash deposit']");
                                NAPI.Data.SetEntityData(player, "ATMTYPE", index);
                                break;
                            case 1:
                                Trigger.ClientEvent(player,
                                    "atmOpen", "[2,0,'Amount for removal']");
                                NAPI.Data.SetEntityData(player, "ATMTYPE", index);
                                break;
                            case 2:
            ...
            try
            {
                ...
                switch (act)
                {
                    ...
                    case 3: //put biz tax
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "An error has arisen!try again.", 3000);
                            return;
                        }
                        int bid = 0;
