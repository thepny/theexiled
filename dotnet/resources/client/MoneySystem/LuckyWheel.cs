    ...
    class LuckyWheel : Script
    {
        ...
        private static void ComeToLuckyWheel(Player player)
        {
            if (DateTime.Now < WaitFor)
            {
                ...
                player.SendNotification("You need to wait a bit");
                return;
            }
            else if (!MoneySystem.Wallet.ChangeLuckyWheelSpins(player, -1))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have no scrolling attempts!", 3000);
                return;
            }
            else
            {
            ...
            if (player.HasSharedData("LUCKY_WHEEL_WIN") && player.HasSharedData("LUCKY_WHEEL_CALL"))
            {
                ...
                switch (player.GetSharedData<int>("LUCKY_WHEEL_WIN"))
                {
                    ...
                    case 19:
                        ...
                        resultName = $"Gaming currency in size {price}$";
                        MoneySystem.Wallet.Change(player, price);
                        break;
                    case 18:
                        resultName = "Exclusive machine";
                    ...
                    case 15:
                        ...
                        break;
                    case 11:
                        resultName = "Unique suit";
                        GiveOutPrizeCostume(player);
                        break;
                    case 4:
                        int donateCoins = Rnd.Next(50, 500);
                        resultName = $"Donat currency in size {donateCoins} RedBucks";
                        MoneySystem.Wallet.ChangeDonateBalance(player, donateCoins);
                        break;
                }
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Win: {resultName}. Congratulations!", 3000);
        ...
        private static void GiveOutPrizeWeapon(Player player)
        {
            ...
            switch (randomInt)
            {
                case 0:
                    ...
                    {
                        Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, $"Not enough Places, You are issued compensation {amountCompensation}$", 3000);
        ...
        private static void GiveOutPrizeVehicle(Player player)
        {
            ...
            {
                MoneySystem.Wallet.Change(player, amountCompensation);
                Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, $"You received Compensation in size {amountCompensation}$ Since you have the maximum number of cars", 3000);
            }
            else
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You received Unique car {model}.", 3000);
            }
        }
        private static void GiveOutPrizeClothes(Player player)
        {
            ...
            if (tryAdd == -1 || tryAdd > 0)
            {
                Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, $"Not enough Places, you issued compensation {amountCompensation}$", 3000);
                MoneySystem.Wallet.Change(player, amountCompensation);
                return;
            }
