...
namespace NeptuneEvo.Families
{
    public class Manager : Script
    {
        ...
        public void StartFamilyManager()
        {
            ...
            try
            {
                blip = NAPI.Blip.CreateBlip(675, PositionFamilyCreator, 0.7f, 55, "Grouping", shortRange: true, dimension: 0);
        ...
        public static void InvitePlayerToFamily(Player player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no place in the family for new participants.Limit: {family.MaxPlayers}", 3000);
                    return;
                }
                ...
                if (Int32.TryParse(st, out id))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Citizen is in a coma", 3000);
                        return;
                    }
                    if (target == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Citizen with such ID not found", 3000);
                        return;
                    }
        ...
        public static void ChangeFamilyRank(Player player, string data1, string data2)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (target == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Citizen with this name not found", 3000);
                        return;
                    }
                    Commands.SetFamilyRank(player, target, Convert.ToInt32(data2));
                }
        ...
        public static void SaveSettingsToFamily(Player player, string desc_1, string desc_2)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You do not have access", 2000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Settings successfully saved", 2000);
            ...
            try
            {
                ...
                if (ranks != null)
                {
                    ...
                    if (acc.FamilyRank == family.AllRanks.Count)
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Range settings are saved", 2000);

                    }
                    else
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "No access", 2000);
                    }
                }
            }
            catch (Exception e)
                ...
                if (family.Leader == acc.UUID)
                {
                    ...
                    foreach (Player p in Members.Keys.ToList())
                    {
                        if (Main.Players[p].FamilyCID == family.FamilyCID)
                        {
                            ...
                            Notify.Send(p, NotifyType.Info, NotifyPosition.BottomCenter, $"A family {family.Name}, It was dissolved by the leader: {player.Name}. {msg}", 2000);
                        }
                    }
        ...
        public static void CreateFamily(Player player, string name, int maxplayers, string img)
        {
            try
            {
                ...
                {
                    case 1:
                        msg = $"You do not have your own personal home";
                        break;
                    case 2:
                        msg = $"You have no money enough";
                        break;
                    case 3:
                        msg = $"Your level is below the required";
                        break;
                    case 4:
                        msg = $"You are already in the family: {Family.GetFamilyName(player)}";
                        break;
                    default:
                        msg = $"A family {name} Successfully created";
                        break;
                }
