...
namespace NeptuneEvo.Houses
{
    ...
    class House
    {
        ...
        public void UpdateLabel()
        {
            try
            {
                string text = $"~w~House ~b~#{ID}\n\n~w~A type: ~b~{HouseManager.HouseTypeList[Type].Name}\n";
                if (!string.IsNullOrEmpty(Owner)) text += $"~w~Owner: ~b~{Owner.Replace('_', ' ')}\n";
                else text += $"~w~Price: ~b~{Price}$\n";
                if (GarageID != 0) text += $"~w~Garage places: ~b~{GarageManager.GarageTypes[GarageManager.Garages[GarageID].Type].MaxCars}\n";
                text += (Locked) ? "~b~Closed\n" : "~b~Open\n";
        ...
        public void CreateFurniture(HouseFurniture f)
        {
            try
            {
                ...
                if (f.Name == "Weapon safe") NAPI.Data.SetEntitySharedData(obj, "TYPE", "WeaponSafe");
                else if (f.Name == "Wardrobe") NAPI.Data.SetEntitySharedData(obj, "TYPE", "ClothesSafe");
                else if (f.Name == "Cabinet with objects") NAPI.Data.SetEntitySharedData(obj, "TYPE", "SubjectSafe");
                Objects.Add(obj);
            }
            catch
            {
        ...
        public void SetOwner(Player player)
        {
            ...
            foreach (var r in Roommates)
            {
                ...
                if (roommate != null)
                {
                    Notify.Send(roommate, NotifyType.Warning, NotifyPosition.BottomCenter, "You were evicted from home", 3000);
    ...
    class HouseManager : Script
    {
        ...
        {
            // name, position
            new HouseType("Auto", new Vector3(1973.124, 3816.065, 32.30873), new Vector3(), 0.0f, "trevorstrailer"),
            new HouseType("Economy", new Vector3(151.2052, -1008.007, -100.12), new Vector3(), 0.0f,"hei_hw1_blimp_interior_v_motel_mp_milo_"),
            new HouseType("Economy+", new Vector3(265.9691, -1007.078, -102.0758), new Vector3(), 0.0f,"hei_hw1_blimp_interior_v_studio_lo_milo_"),
                        ...
                        else
                        {
                            if (house.Locked)
                            {
                                ...
                                if (playerHouse != null && playerHouse.ID == house.ID)
                                    ...
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have access", 3000);
                            }
                            else
                                house.SendPlayer(player);
                        }
        ...
        private static void callback_housebuy(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "buy":
                    ...
                    if (!string.IsNullOrEmpty(house.Owner))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is already a master in this house.", 3000);
                        return;
                    }

                    if (house.Price > Main.Players[player].Money)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough money to buy a house", 3000);
                        return;
                    }
                    ...
                    if (vehicles > maxcars)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The house you buy has {maxcars} Machinesture, sell extra cars", 3000);
                        OpenCarsSellMenu(player);
                        return;
                    }
                    if (HouseTypeList[house.Type].PetPosition != null) house.PetName = Main.Players[player].PetName;
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought this house, do not forget to make a tax for it in an ATM", 3000);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.Center, $"Do not forget to make taxes for the house in the nearest ATM!", 8000);
                    CheckAndKick(player);
                    house.SetLock(true);
        ...
        public static void OpenHouseManageMenu(Player player)
        {
            ...
            if (house == null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no house", 3000);
                MenuManager.Close(player);
                return;
            }
            ...
            Menu.Item menuItem = new Menu.Item("header", Menu.MenuItem.Header);
            menuItem.Text = "House management";
            menu.Add(menuItem);
            ...
            menuItem.Text = "Spearing";
            menu.Add(menuItem);

            menuItem = new Menu.Item("sell", Menu.MenuItem.Button);
            menuItem.Text = $"Sell a state for {Convert.ToInt32(house.Price * 0.6)}$";
            menu.Add(menuItem);
        ...
        private static void callback_housemanage(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                ...
                case "removeall":
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You kicked out all from home", 3000);
                    return;
                case "furniture":
                    MenuManager.Close(player);
                ...
                case "sell":
                    ...
                    Trigger.ClientEvent(player, "openDialog", "HOUSE_SELL_TOGOV", $"You really want to sell a house for ${price}?");
                    MenuManager.Close(player);
                    return;
                case "cars":
                    OpenCarsMenu(player);
        ...
        public static void acceptHouseSellToGov(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must get out of the garage", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You sold your home to the state for {price}$", 3000);
        }
        ...
        public static void OpenCarsSellMenu(Player player)
        {
            ...
            Menu.Item menuItem = new Menu.Item("header", Menu.MenuItem.Header);
            menuItem.Text = "Cars sale";
            menu.Add(menuItem);

            menuItem = new Menu.Item("label", Menu.MenuItem.Card);
            menuItem.Text = "Choose the machine you want to sell";
            menu.Add(menuItem);
        ...
        private static void callback_carsell(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You sold {vData.Model} ({item.ID}) for {price}$", 3000);
        ...
        public static void OpenFurnitureMenu(Player player)
        {
            ...
            menuItem.Text = "Purchase furniture";
            menu.Add(menuItem);

            menuItem = new Menu.Item("tofurniture", Menu.MenuItem.Button);
            menuItem.Text = "Managing furniture";
            menu.Add(menuItem);
        ...
        private static void callback_furniture0(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            else if (item.ID == "buyfurniture")
            {
                ...
                menuItem.Text = "Armory safe (15000$)";
                nmenu.Add(menuItem);

                menuItem = new Menu.Item("buy2", Menu.MenuItem.Button);
                menuItem.Text = "Wardrobe (15000$)";
                nmenu.Add(menuItem);

                menuItem = new Menu.Item("buy3", Menu.MenuItem.Button);
                menuItem.Text = "Cabinet with objects (15000$)";
                nmenu.Add(menuItem);
        ...
        private static void callback_furniture1(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "In your apartment already too many furniture, sell something", 3000);
                return;
            }
            if (item.ID == "buy1")
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You do not have enough money to buy this furniture..", 3000);
                    return;
                }
                MoneySystem.Wallet.Change(player, -15000);
                FurnitureManager.newFurniture(house.ID, "Weapon safe");
                GameLog.Money("server", $"player({Main.Players[player].UUID})", 15000, $"buyFurn({house.ID} | Armory safe)");
            }
            else if (item.ID == "buy2")
            {
                ...
                MoneySystem.Wallet.Change(player, -15000);
                FurnitureManager.newFurniture(house.ID, "Wardrobe");
                GameLog.Money("server", $"player({Main.Players[player].UUID})", 15000, $"buyFurn({house.ID} | Wardrobe)");
            }
            else if (item.ID == "buy3")
            {
                ...
                MoneySystem.Wallet.Change(player, -15000);
                FurnitureManager.newFurniture(house.ID, "Cabinet with objects");
                GameLog.Money("server", $"player({Main.Players[player].UUID})", 15000, $"buyFurn({house.ID} | Cabinet with objects)");
            }
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Congratulations on the successful purchase of furniture!", 3000);
            MenuManager.Close(player);
        }
        ...
        private static void callback_furniture(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be at home", 3000);
                MenuManager.Close(player);
                return;
            }
            if (!FurnitureManager.HouseFurnitures.ContainsKey(house.ID) || FurnitureManager.HouseFurnitures[house.ID].Count() == 0)
            ...
            if (item.ID == "sellit")
            {
                if (f.IsSet)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Remove furniture before selling.", 3000);
                    return;
                }
                GameLog.Money($"player({Main.Players[player].UUID})", "server", 7500, $"sellFurn({house.ID} | {f.Name})");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have sold successfully {f.Name} for 7500$", 3000);
            ...
            switch (eventName)
            {
                case "button":
                    switch (f.IsSet)
                    {
                        case true:
                            house.DestroyFurniture(f.ID);
                            f.IsSet = false;
                            menu.Items[4].Text = $"Installed: No.";
                ...
                case "listChangeright":
                    ...
                    menu.Items[3].Text = $"A type: {f.Name}";
                    menu.Change(player, 3, menu.Items[3]);

                    var open = (f.IsSet) ? "Yes" : "Not";
                    menu.Items[4].Text = $"Installed: {open}";
                    menu.Change(player, 4, menu.Items[4]);
                    return;
            }
        }
        ...
        public static void OpenRoommatesMenu(Player player)
        {
            ...
            {
                menuItem = new Menu.Item("label", Menu.MenuItem.Card);
                menuItem.Text = "Click on the name of the player you want to evict";
                menu.Add(menuItem);
            ...
            {
                menuItem = new Menu.Item("label", Menu.MenuItem.Card);
                menuItem.Text = "You have no one is sent to the house";
                menu.Add(menuItem);
            }
        ...
        private static void callback_roommates(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You evicted {mName} From your house", 3000);
        }
        ...
        private static void callback_selectedcar(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                case "sell":
                    ...
                    Trigger.ClientEvent(player, "openDialog", "CAR_SELL_TOGOV", $"You really want to sell the state {vData.Model} ({menu.Items[0].Text}) for ${price}?");
                    MenuManager.Close(player);
                    return;
                case "repair":
                    ...
                    if (vData.Health > 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The machine does not need to restore", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You restored {vData.Model} ({menu.Items[0].Text})", 3000);
                    return;
                case "evac":
                    ...
                    if (!check)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This car is in the garage", 3000);
                        return;
                    }
                    if (Main.Players[player].Money < 200)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Not enough funds (not enough {200 - Main.Players[player].Money}$)", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Your car was removed to the garage", 3000);
                    return;
                case "evac_pos":
                    if (!Main.Players.ContainsKey(player)) return;
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The machine does not need evacuation", 3000);
                        return;
                    }
                    ...
                    garage = GarageManager.Garages[GetHouse(player).GarageID];
                    garage.SendVehicleIntoGarage(number);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Your car was evacuated in the garage", 3000);
                    return;
                case "gps":
                    if (!Main.Players.ContainsKey(player)) return;
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "The GPS marked the location of your car", 3000);
                    return;
                case "key":
                    if (!Main.Players.ContainsKey(player)) return;
                    ...
                    else
                    {
                        if (Main.Players[player].InsideGarageID == -1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be in the garage", 3000);
                            return;
                        }
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You received Machine key with number {menu.Items[0].Text}", 3000);
                    return;
                case "changekey":
                    if (!Main.Players.ContainsKey(player)) return;
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Change of locks worth $1000", 3000);
                        return;
                    }

                    VehicleManager.Vehicles[menu.Items[0].Text].KeyNum++;
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You changed the locks by car {menu.Items[0].Text}. Now old keys cannot be used.", 3000);
                    return;
            }
        }
        #endregion
        ...
        public static void InviteToRoom(Player player, Player guest)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have a maximum number of players in your house", 3000);
                return;
            }

            if (GetHouse(guest) != null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player already lives in the house", 3000);
                return;
            }

            guest.SetData("ROOM_INVITER", player);
            guest.TriggerEvent("openDialog", "ROOM_INVITE", $"Player ({player.Value}) suggested that you press it");

            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered a player ({guest.Value}) Hatch to you", 3000);
        }
        ...
        public static void acceptRoomInvite(Player player)
        {
            ...
            if (house.Roommates.Count >= MaxRoommates[house.Type])
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The house has the maximum number of players", 3000);
                return;
            }
            ...
            Notify.Send(owner, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) Halfed to you", 3000);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You sat down to the player ({owner.Value})", 3000);
        }
        ...
        public static void CMD_InvitePlayerToHouse(Player player, int id)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are too far", 3000);
                return;
            }
        ...
        public static void InvitePlayerToHouse(Player player, Player guest)
        {
            ...
            Notify.Send(guest, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) invited you to your home", 3000);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You invited players ({guest.Value}) to his house", 3000);
        }
        ...
        public static void OfferHouseSell(Player player, Player target, int price)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are too far from the buyer", 3000);
                return;
            }
            House house = GetHouse(player, true);
            if (house == null)
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player already has a house", 3000);
                return;
            }
            if (price > 1000000000 || price < house.Price / 2)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Too big / small price", 3000);
                return;
            }
            if (player.Position.DistanceTo(house.Position) > 30)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are too far from home", 3000);
                return;
            }

            target.SetData("HOUSE_SELLER", player);
            target.SetData("HOUSE_PRICE", price);
            Trigger.ClientEvent(target, "openDialog", "HOUSE_SELL", $"Player ({player.Value}) suggested you buy your home for ${price}");
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered a player ({target.Value}) buy your home for {price}$", 3000);
        }
        ...
        public static void acceptHouseSell(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You already have a house", 3000);
                return;
            }
            ...
            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) bought you home", 3000);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You bought a house with a player ({seller.Value})", 3000);
        }
        #endregion
    }
}
