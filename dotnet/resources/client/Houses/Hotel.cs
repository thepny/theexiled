...
namespace NeptuneEvo.Houses
{
    class Hotel : Script
    {
        ...
        public static void ExtendHotelRent(Player player, int hours)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You are not set in any hotel", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You extended rental on {hours} hrs, You will be evicted through {Main.Players[player].HotelLeft} hrs", 3000);
        }
        ...
        public static void OpenHotelBuyMenu(Player player)
        {
            ...
            menuItem.Text = $"Money for rent will be filmed every Peiley only when you are in the game.";
            menu.Add(menuItem);

            menuItem = new Menu.Item("rent", Menu.MenuItem.Button);
            menuItem.Text = $"Rent ({HotelRent}$)";
            menu.Add(menuItem);
        ...
        private static void callback_hotelbuy(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "rent":
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You live in the house and can not rent a room in the hotel", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You rented a room at the hotel for 1.You can extend the rent in the phone (M)", 3000);
                    SendToRoom(player);
        ...
        public static void OpenHotelManageMenu(Player player)
        {
            ...
            menuItem = new Menu.Item("info", Menu.MenuItem.Card);
            menuItem.Text = $"Rental paid by {Main.Players[player].HotelLeft}ч";
            menu.Add(menuItem);

            menuItem = new Menu.Item("extend", Menu.MenuItem.Button);
            menuItem.Text = "Extend rent.";
            menu.Add(menuItem);
        ...
        private static void callback_hotelmanage(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "extend":
                    MenuManager.Close(player);
                    Trigger.ClientEvent(player, "openInput", $"Extend rent. ({HotelRent}$/ hrs)", "Enter the number of hours", 1, "extend_hotel_rent");
                    return;
                case "moveout":
                    MoveOutPlayer(player);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "You searched from the hotel", 3000);
                    MenuManager.Close(player);
                    return;
                case "close":
                    MenuManager.Close(player);
