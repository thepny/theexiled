...
namespace NeptuneEvo.Fractions
{
    class Gangs : Script
    {
        ...
        public static void InteractPressed(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You must be in the car that can transport drugs", 3000);
                return;
            }
            if (Fractions.Manager.FractionTypes[Main.Players[player].FractionID] != 1)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You can't buy drugs", 3000);
                return;
            }
            if (!Fractions.Manager.canUseCommand(player, "buydrugs")) return;
            Trigger.ClientEvent(player, "openInput", "Buy drugs", $"Enter the number of:", 4, "buy_drugs");
        }
        ...
        public static void BuyDrugs(Player player, int amount)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Not enough Places in the car", 3000);
                return;
            }
            if (Fractions.Stocks.fracStocks[Main.Players[player].FractionID].Money < amount * PricePerDrug)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough Tools in stock gang", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You purchased {amount} Gram of drugs", 3000);
        }
    }
}
