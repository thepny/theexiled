    ...
    class Fbi : Script
    {
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                ...
                case 24:
                    if (Main.Players[player].FractionID != 9)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not a FBI employee", 3000);
                        return;
                    }
                ...
                case 46:
                    ...
                    if (player.GetData<bool>("IN_CP_MODE"))
                    {
                        ...
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You changed into the working form", 3000);
                    }
                    else
                    {
                        if (!warg_mode)
                        ...
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You changed in special.Form", 3000);
                    }
                    return;
                case 61:
                    if (Main.Players[player].FractionID != 9)
                    ...
                    GUI.Dashboard.OpenOut(player, Stocks.fracStocks[9].Weapons, "Armory", 6);
                    return;
            }
        }
        ...
        public static void callback_fbilift(Player client, int floor)
        {
            try
            {
                ...
                {
                    Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"Someone drags you", 3000);
                    return;
                }
                switch (floor)
                {
            ...
            try
            {
                switch (index)
                {
                    ...
                    case 5:
                        ...
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough Materials in warehouse", 3000);
                            return;
                        }
