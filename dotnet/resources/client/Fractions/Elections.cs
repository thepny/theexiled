    ...
    class ElectionsSystem : Script
    {
        ...
        public static void Interaction(ColShape colshape, Player player)
        {
            try
            {
                ...
                {
                    player.SendChatMessage($"Your level should be at least {minVoteLVL}LVL!");
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, $"Your level should be at least {minVoteLVL}LVL!", 3000);
                    return;
                }
                    ...
                    if (ElectionPointsList[i].Point == colshape)
                    {
                        if (ElectionPointsList[i].Opened)
                        {
                            if (ElectionList.Count >= 2)
                            {
                                ...
                                else player.SendChatMessage("You have already voted from this account in these elections.");
                            }
                            else player.SendChatMessage("No candidates for these elections not found.");
                        }
                        break;
                    }
                }
                            ...
                            for (int l = 0; l != ElectionList.Count; l++)
                            {
                                ...
                                { // Теперь ищем игроков с номером выборов, которые подходят нам
                                    ...
                                    { // Если имя то, которое мы выбрали, то добавляем ему голос.
                                        ...
                                        player.SendChatMessage($"You made your voice in the election № {ElectionList[l].Election} in favor {Name}");

                                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                            $"You made your voice in the election № {ElectionList[l].Election} in favor {Name}", 3000);
                                        break;
                                    }
                                }
                            }
        ...
        public void AddPoint_CMD(Player client, uint ElectionID, bool isOpened = false)
        {
            ...
            {
                client.SendChatMessage("Such election already exists!");
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Such election already exists!", 3000);
                return;
            }
