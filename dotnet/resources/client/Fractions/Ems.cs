    ...
    class Ems : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Click Е"), new Vector3(emsCheckpoints[0].X, emsCheckpoints[0].Y, emsCheckpoints[0].Z + 1), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Click Е"), new Vector3(emsCheckpoints[1].X, emsCheckpoints[1].Y, emsCheckpoints[1].Z + 1), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Open warehouse"), new Vector3(emsCheckpoints[3].X, emsCheckpoints[3].Y, emsCheckpoints[3].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));

                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[4], 1, 2, 0); // duty change
                col.SetData("INTERACT", 18);
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Click Е, To change clothes"), new Vector3(emsCheckpoints[4].X, emsCheckpoints[4].Y, emsCheckpoints[4].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));

                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[5], 1, 2, 0); // start heal course
                col.SetData("INTERACT", 19);
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Click Е, To begin treatment"), new Vector3(emsCheckpoints[5].X, emsCheckpoints[5].Y, emsCheckpoints[5].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                NAPI.Blip.CreateBlip(305, new Vector3(3588.917, 3661.756, 41.48687), 1, 3, "Composition of the pharmacy", 255, 0, true);
                #endregion
                ...
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~On the roof"), new Vector3(emsCheckpoints[8].X, emsCheckpoints[8].Y, emsCheckpoints[8].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));
        ...
        public static void callEms(Player player, bool death = false)
        {
            if (!death)
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "No doctors in your area.try later", 3000);
                    return;
                }
                if (player.HasData("NEXTCALL_EMS") && DateTime.Now < player.GetData<DateTime>("NEXTCALL_EMS"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have already called physicians, try later", 3000);
                    return;
                }
            ...
            Manager.sendFractionMessage(8, $"Received a call from the player ({player.Value})");
            Manager.sendFractionMessage(8, $"~b~Received a call from the player ({player.Value})", true);
        }
        ...
        public static void acceptCall(Player player, Player target)
        {
            ...
            try
            {
                ...
                {
                    where = 2;
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The player did not cause EMS, or this challenge has already accepted", 3000);
                    return;
                }
                where = 3;
                ...
                Manager.sendFractionMessage(7, $"{player.Name.Replace('_', ' ')} accepted a call from the player ({target.Value})");
                where = 9;
                Manager.sendFractionMessage(7, $"~b~{player.Name.Replace('_', ' ')} accepted a call from the player ({target.Value})", true);
                where = 10;
        ...
        public static void onPlayerDisconnectedhandler(Player player, DisconnectionType type, string reason)
        {
            try
            {
                ...
                if (player.HasData("CALLEMS_BLIP"))
                {
                    ...
                    Manager.sendFractionMessage(8, $"{player.Name.Replace('_', ' ')} canceled call");
                }
            ...
            try
            {
                ...
                if (!player.HasData("IS_DYING"))
                {
                    ...
                    else
                    {
                        ...
                        Trigger.ClientEvent(player, "openDialog", "DEATH_CONFIRM", $"You want to cause physicians ({medics} online)?");
                    }
                }
                else
                {
        ...
        public static void DeathConfirm(Player player, bool call)
        {
            ...
            var timeMsg = (call) ? "10 minutes You will not cure medic or someone else" : "3 minutes Nobody will cure you";
            ...
            Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, $"If within {timeMsg}, then you will get to the hospital", 3000);
        }
        ...
        public static void payMedkit(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no so much money", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought the first-aid kit", 3000);
            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) I bought a first-aid kit", 3000);
        }
        ...
        public static void payHeal(Player player)
        {
            ...
            if (NAPI.Player.IsPlayerInAnyVehicle(seller) && NAPI.Player.IsPlayerInAnyVehicle(player))
            {
                ...
                Notify.Send(seller, NotifyType.Success, NotifyPosition.BottomCenter, $"You cured players ({player.Value})", 3000);
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                ...
                case 16:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must first finish the treatment", 3000);
                        break;
                    }
                    /*if (player.HasData("HEAL_TIMER"))
                    {
                ...
                case 19:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not need treatment", 3000);
                        break;
                    }
                    if (player.HasData("HEAL_TIMER"))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are already treated", 3000);
                        break;
                    }
                ...
                case 58:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not in the car or your car can not carry first aid kits", 3000);
                        break;
                    }
                    ...
                    if (medCount >= 50)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"In the car maximum aid kits", 3000);
                        break;
                    }
                    if (HumanMedkitsLefts <= 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"First aid kits ended.Come for new in an hour", 3000);
                        break;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You filled the car aid kits", 3000);
                    return;
                case 63:
                    if (Main.Players[player].FractionID != 8)
                    {
            ...
            NAPI.Task.Run(() =>
            {
                try
                {
                    if (player.Health == 100)
                    {
                        ...
                        player.ResetData("HEAL_TIMER");
                        Trigger.ClientEvent(player, "stopScreenEffect", "PPFilter");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Your treatment is completed", 3000);
                        return;
                    }
        ...
        public static void OpenHospitalStockMenu(Player player)
        {
            ...
            menuItem.Text = $"Storage ({Stocks.fracStocks[8].Medkits}шт)";
            menu.Add(menuItem);

            menuItem = new Menu.Item("takemed", Menu.MenuItem.Button);
            menuItem.Text = "Take a first-aid kit";
            menu.Add(menuItem);

            menuItem = new Menu.Item("putmed", Menu.MenuItem.Button);
            menuItem.Text = "Put a first-aid kit";
            menu.Add(menuItem);

            menuItem = new Menu.Item("tazer", Menu.MenuItem.Button);
            menuItem.Text = "Take electric shock";
            menu.Add(menuItem);
        ...
        private static void callback_hospitalstock(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "takemed":
                    ...
                    {
                        Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"Intertech left in stock", 3000);
                        return;
                    }
                    ...
                    Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, $"You took the first-aid kit.You {itemInv.Count} units", 3000);
                ...
                case "putmed":
                    ...
                    if (itemInv == null)
                    {
                        Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no aidhechki", 3000);
                        return;
                    }
                    nInventory.Remove(client, ItemType.HealthKit, 1);
                    Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, $"You put a first-aid kit.You have left {itemInv.Count - 1} units", 3000);
                    Stocks.fracStocks[8].Medkits++;
                ...
                case "tazer":
                    ...
                    {
                        Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have access to stucker", 3000);
                        return;
                    }
            ...
            Menu.Item menuItem = new Menu.Item("header", Menu.MenuItem.Header);
            menuItem.Text = $"Storage ({Stocks.fracStocks[8].Medkits} units)";
        ...
        public static void OpenTattooDeleteMenu(Player player)
        {
            ...
            Menu.Item menuItem = new Menu.Item("header", Menu.MenuItem.Header);
            menuItem.Text = $"Bearing tattoos";
            menu.Add(menuItem);

            menuItem = new Menu.Item("header", Menu.MenuItem.Card);
            menuItem.Text = $"Select the zone in which you want to reduce all tattoos.The cost of information in one zone - 3000$";
            menu.Add(menuItem);
        ...
        private static List<string> TattooZonesNames = new List<string>() { "if it takes up", "Head", "left hand", "right hand", "Left leg", "Right leg" };
        private static void callback_tattoodelete(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            {
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "You do not have tattoos in this zone", 3000);
                return;
            }
            if (!MoneySystem.Wallet.Change(client, -600))
            {
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough money", 3000);
                return;
            }
            ...
            Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, "You brought tattoos with " + TattooZonesNames[Convert.ToInt32(zone)], 3000);
        }
        #endregion
    }
}
