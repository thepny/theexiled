...
namespace NeptuneEvo.Fractions
{
    class Configs : Script
    {
        ...
        private static void DeleteSpawnedCar(Player player, string vnumber)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough level", 2500);
                return;
            }
        ...
        private static void SpawnFractionCar(Player player, string vnumber)
        {
            ...
            if (player.HasData("LASTSPAWNCAR"))
            {
                ...
                if (DateTime.Now < lastSpawn)
                {
                    ...
                    var min = g.Minute;
                    var sec = g.Second;
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can call any car through {min}:{sec}", 3000);
                    return;
                }
            }
            if (player.HasData($"{vnumber}SPAWN"))
            {
                ...
                if (DateTime.Now < lastSpawn)
                {
                    ...
                    var min = g.Minute;
                    var sec = g.Second;
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can call this machine through {min}:{sec}", 3000);
                    return;
                }
            }
    ...
    public class CarSpawner
    {
        ...
        private void Create()
        {
            ...
            label = NAPI.TextLabel.CreateTextLabel("Vehicle spawn", Position + new Vector3(0, 0, 1), 10f, 5f, 4, new Color(255, 255, 255), false, 0);
