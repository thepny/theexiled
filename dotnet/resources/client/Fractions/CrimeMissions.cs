    ...
    class CarDelivery : Script
    {
            ...
            try
            {
                ...
                colShape.OnEntityEnterColShape += (s, e) =>
                {
                    try
                    {
                        ...
                        Notify.Send(e, NotifyType.Success, NotifyPosition.BottomCenter, "You arrested the car", 3000);
                    }
                ...
                foreach (var pos in GangEndDelivery)
                {
                    ...
                    colShape.OnEntityEnterColShape += (s, e) =>
                    {
                        try
                        {
                            ...
                            e.SendChatMessage("Cutting machine: !{#00FF00}500$ ~w~were sent to the bump of gang. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()}" + ")");
                        }
                ...
                foreach (var pos in MafiaEndDelivery)
                {
                    ...
                    colShape.OnEntityEnterColShape += (s, e) =>
                    {
                        try
                        {
                            ...
                            Stocks.fracStocks[Main.Players[e].FractionID].Money += 500;
                            GameLog.Money($"server", $"frac({Main.Players[e].FractionID})", 500, "dropCar");
                            e.SendChatMessage("Delivery of the Fourgon: !{#00FF00}500$ ~w~were sent to the Mafia casket. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()})");
                        }
            ...
            try
            {
                ...
                switch (id)
                {
                    ...
                    case 1:
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "You got the coordinates of the car.Sit in it and take to the place, which is marked in the GPS of the car.", 3000);
                        return;
                }
            }
        ...
        public void Event_mafiaMission(Player player, int id)
        {
            try
            {
                ...
                NextDelivery[fraction] = DateTime.Now.AddMinutes(5);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You got a van with {text} For transportation.Take it to the map specified on the map", 3000);
                Trigger.ClientEvent(player, "createWaypoint", MafiaEndDelivery[end].X, MafiaEndDelivery[end].Y);
