...
namespace NeptuneEvo.Fractions
{
    ...
    { // Revision 3.0
        ...
        public static void onResourceStart()
        {
            try
            {
                ...
                NAPI.Blip.CreateBlip(419, Cityhall.CityhallChecksCoords[2], 1.5f, 14, Main.StringToU16("City Hall"), 255, 0, true, 0, 0);
                ...
                NAPI.Blip.CreateBlip(85, Army.ArmyCheckpoints[2], 1, 28, Main.StringToU16("Until"), 255, 0, true, 0, 0); // Армия в доках (загрузка)
                NAPI.Blip.CreateBlip(76, Army.ArmyCheckpoints[0], 1, 28, Main.StringToU16("Until"), 255, 0, true, 0, 0);
                ...
                NAPI.Blip.CreateBlip(197, new Vector3(-1036.3226, -2732.918, 12.636636), 1, 26, Main.StringToU16("New players spawn"), 255, 0, true, 0);
        ...
        public static void giveGun(Player player, Weapons.Hash gun, string weaponstr)
        {
            ...
            if (player.HasData($"GET_{gun.ToString()}") && DateTime.Now < player.GetData<DateTime>($"GET_{gun.ToString()}"))
            {
                ...
                var min = g.Minute;
                var sec = g.Second;
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can take {gun.ToString()} through {min}:{sec}", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have access to this weapon", 3000);
                return;
            }
            if (Stocks.fracStocks[Main.Players[player].FractionID].Materials < matsForGun[gun])
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"In stock insufficient material", 3000);
                return;
            }
        ...
        public static void giveAmmo(Player player, ItemType ammoType, int ammo)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You received {nInventory.ItemsNames[(int)ammoType]} x{ammo}", 3000);
            return;
        }
        ...
        public static void OpenGunCraftMenu(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You do not know how to craft weapons", 3000);
                return;
            }
        ...
        public static void Event_WCraft(Player client, int frac, int cat, int index)
        {
            ...
            try
            {
                ...
                {
                    Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "You do not know how to craft weapons", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "You have insufficient material", 3000);
                    return;
                }
                ...
                Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, $"You are scratchy {selected} for {matsForGun[Weapons.GetHash(selected)]} matt", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void Event_WCraftAmmo(Player player, int frac, string text1, string text2)
        {
            try
            {
                ...
                if (ammo == 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not indicate the number of cartridge", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough materials", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have successfully scratching {nInventory.ItemsNames[(int)AmmoTypes[category]]} x{ammo}", 3000);
            }
                ...
                switch (act)
                {
                    case 2: //invite
                        ...
                        else
                        {
                            ...
                            if (target == null)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player with the same name was not found", 3000);
                                return;
                            }
                            FractionCommands.InviteToFraction(player, target);
                        }
                    ...
                    case 3: //kick
                        ...
                        else
                        {
                            ...
                            if (target == null)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player does not consist in your fraction", 3000);
                                    return;
                                }
                                if (targetLvl >= Main.Players[player].FractionLVL)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't dismiss this player", 3000);
                                    return;
                                }
                                MySQL.Query($"UPDATE `characters` SET fraction=0,fractionlvl=0 WHERE firstname='{split[0]}' AND lastname='{split[1]}'");
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You fired player {data1} From your fraction", 3000);
    ...
    class Stocks : Script
    {
        ...
        public static void inputStocks(Player player, int where, string action, int amount)
        {
                ...
                switch (action)
                {
                    case "put_stock":
                        ...
                        if (item == "mats")
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Warehouse can not accommodate such a number of material", 3000);
                                return;
                            }
                        ...
                        else if (item == "drugs")
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Warehouse can not accommodate such a number of drugs", 3000);
                                return;
                            }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no so much", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"The warehouse remains {stockContains + amount}, in you {playerHave - amount}", 3000);
                        data.UpdateLabel();
                        break;
                    case "take_stock":
                        ...
                        if (item == "mats")
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't accommodate so much in your inventory.", 3000);
                                return;
                            }
                        }
                        else if (item == "drugs")
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There are so many stock", 3000);
                            return;
                        }
            ...
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The car can not carry anything", 3000);
                    return;
                }
                ...
                switch (action)
                {
                    case "load_mats":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no such number of mats in stock", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You downloaded materials in the car", 3000);
                    ...
                    case "unload_mats":
                        ...
                        fracStocks[onfrac].Materials += amount;
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You unloaded materials from the car", 3000);
                        fracStocks[onfrac].UpdateLabel();
                    ...
                    case "load_drugs":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not a member ~y~{Fractions.Manager.getName(player.GetData<int>("ONFRACSTOCK"))}", 3000);
                            return;
                        }
                        if (!Manager.canUseCommand(player, "takestock")) return;
                        if (fracStocks[onfrac].Drugs < amount)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no such number of drugs in stock", 3000);
                            return;
                        }
                        tryAdd = VehicleInventory.TryAdd(vehicle, new nItem(ItemType.Drugs, amount));
                        if (tryAdd == -1 || tryAdd > 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"It is impossible to upload such a number of drugs", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You downloaded drugs in the car", 3000);
                        fracStocks[onfrac].UpdateLabel();
                    ...
                    case "unload_drugs":
                        ...
                        if (count < amount)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no such number of drugs in the car", 3000);
                            return;
                        }
                        ...
                        fracStocks[onfrac].Drugs += amount;
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You discharge drugs from the car", 3000);
                        fracStocks[onfrac].UpdateLabel();
                    ...
                    case "load_medkits":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no such number of first aid kits", 3000);
                            return;
                        }
                        var maxMedkits = 100;
                        if (VehicleInventory.GetCountOfType(vehicle, ItemType.HealthKit) + amount > maxMedkits)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"It is impossible to download such a number of first-aid kits", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You downloaded first aid kits", 3000);
                        fracStocks[onfrac].UpdateLabel();
                    ...
                    case "unload_medkits":
                        ...
                        if (count < amount)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"In the car there is no such number of first-aid kits", 3000);
                            return;
                        }
                        if (fracStocks[onfrac].Medkits + amount > 1000)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Warehouse can not accommodate such a number of first-aid kits", 3000);
                            return;
                        }
                        VehicleInventory.Remove(vehicle, ItemType.HealthKit, amount);
                        fracStocks[onfrac].Medkits += amount;
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You unloaded the first aid kits", 3000);
                        fracStocks[onfrac].UpdateLabel();
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                case 32:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Warehouse is tightly closed", 3000);
                        return;
                    }
        ...
        public static void Event_openWeaponsStock(Player player)
        {
            try
            {
                ...
                GUI.Dashboard.OpenOut(player, fracStocks[(int)player.GetData<int>("ONFRACSTOCK")].Weapons, "Armory", 6);
            }
        ...
        private static void callback_stockselect(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "put":
                    MenuManager.Close(player);
                    Main.OpenInputMenu(player, "Enter the number of", "put_stock");
                    return;
                case "take":
                    MenuManager.Close(player);
                    Main.OpenInputMenu(player, "Enter the number of", "take_stock");
                    return;
                case "back":
                    MenuManager.Close(player);
                    OpenFracStockMenu(player);
        ...
        public class FractionStock
        {
            ...
            public void UpdateLabel()
            {
                ...
                var text = $"~b~";
                if (Drugs > 0) text += $"Narcotics: {Drugs}/10000\n";
                if (Materials > 0) text += $"Materials: {Materials}/{maxMats}\n";
                if (Medkits > 0) text += $"First aid kit: {Medkits}\n";
                label.Text = text;
            }
        }
    }
    ...
    class MatsWar : Script
    {
        ...
        {
            Manager.sendFractionMessage(1, "After 10 minutes, a ship with materials will arrive in the Los Santos port.");
            Manager.sendFractionMessage(2, "After 10 minutes, a ship with materials will arrive in the Los Santos port.");
            Manager.sendFractionMessage(3, "After 10 minutes, a ship with materials will arrive in the Los Santos port.");
            Manager.sendFractionMessage(4, "After 10 minutes, a ship with materials will arrive in the Los Santos port.");
            Manager.sendFractionMessage(5, "After 10 minutes, a ship with materials will arrive in the Los Santos port.");
            Manager.sendFractionMessage(10, "After 10 minutes, a ship with materials will arrive in the Los Santos port.");
            Manager.sendFractionMessage(11, "After 10 minutes, a ship with materials will arrive in the Los Santos port.");
            Manager.sendFractionMessage(12, "After 10 minutes, a ship with materials will arrive in the Los Santos port.");
            Manager.sendFractionMessage(13, "After 10 minutes, a ship with materials will arrive in the Los Santos port.");
        ...
        public static void startWar()
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    isWar = true;
                    Manager.sendFractionMessage(1, "The ship with the materials arrived at the Los Santos port.");
                    Manager.sendFractionMessage(2, "The ship with the materials arrived at the Los Santos port.");
                    Manager.sendFractionMessage(3, "The ship with the materials arrived at the Los Santos port.");
                    Manager.sendFractionMessage(4, "The ship with the materials arrived at the Los Santos port.");
                    Manager.sendFractionMessage(5, "The ship with the materials arrived at the Los Santos port.");
                    Manager.sendFractionMessage(10, "The ship with the materials arrived at the Los Santos port.");
                    Manager.sendFractionMessage(11, "The ship with the materials arrived at the Los Santos port.");
                    Manager.sendFractionMessage(12, "The ship with the materials arrived at the Los Santos port.");
                    Manager.sendFractionMessage(13, "The ship with the materials arrived at the Los Santos port.");
                    warblip.Color = 49;
        ...
        public static void endWar()
        {
            try
            {
                NAPI.Task.Run(() =>
                {
                    ...
                    isWar = false;
                    Manager.sendFractionMessage(1, "The ship left the port of Los Santos.");
                    Manager.sendFractionMessage(2, "The ship left the port of Los Santos.");
                    Manager.sendFractionMessage(3, "The ship left the port of Los Santos.");
                    Manager.sendFractionMessage(4, "The ship left the port of Los Santos.");
                    Manager.sendFractionMessage(5, "The ship left the port of Los Santos.");
                    Manager.sendFractionMessage(10, "The ship left the port of Los Santos.");
                    Manager.sendFractionMessage(11, "The ship left the port of Los Santos.");
                    Manager.sendFractionMessage(12, "The ship left the port of Los Santos.");
                    Manager.sendFractionMessage(13, "The ship left the port of Los Santos.");
                    warblip.Color = 49;
                });
            }
        ...
        public static void interact(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can not do it", 3000);
                return;
            }
            if (!player.IsInVehicle)
            {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"On this machine can not be transported by mats", 3000);
                return;
            }
            if (player.HasData("loadMatsTimer"))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You already load materials in the car", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"In the car, the maximum number of material", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Loading materials began (20 seconds)", 3000);
            ...
            try
            {
                if (NAPI.Entity.GetEntityType(entity) == EntityType.Player)
                {
                    ...
                    if (entity.IsInVehicle && NAPI.Data.HasEntityData(entity.Vehicle, "loaderMats"))
                    {
                        ...
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"Loading materials canceled, as the machine left Chekpot", 3000);
                    }
                }
            }
