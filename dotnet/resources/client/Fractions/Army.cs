    ...
    class Army : Script
    {
            ...
            try
            {
                switch (index)
                {
                    ...
                    case 8: //Payek
                        ...
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "You already have 2 dry. Pike", 3000);
                            return;
                        }
                        if (Fractions.Stocks.fracStocks[14].Materials > Fractions.Manager.matsForPayek)
                        {
                            ...
                            Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"You got an army dry.", 3000);
                        }
                        return;
                }
            }
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                case 34:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not serve in the army", 3000);
                        return;
                    }
                ...
                case 36:
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Loading materials began (20 seconds)", 3000);
                ...
                case 60: // open stock gun
                    ...
                    GUI.Dashboard.OpenOut(player, Stocks.fracStocks[14].Weapons, "Armory", 6);
                    return;
            }
        }
        ...
        private static void onEntityExitArmyMats(ColShape shape, Player player)
        {
            ...
            if (NAPI.Data.HasEntityData(player, "loadMatsTimer"))
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The loading of materials is canceled, since the machine left Chekpoint", 3000);
            }
        }
            ...
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    if (player.GetData<string>("whereLoad") == "WAR" && !Fractions.MatsWar.isWar)
                    {
                        ...
                        player.ResetData("loadMatsTimer");
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The ship is already left", 3000);
                        return;
                    }
        ...
        public static void Event_PlayerDeath(Player player, Player entityKiller, uint weapon)
        {
            try
            {
                ...
                {
                    Trigger.ClientEvent(player, "hideLoader");
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Loading materials canceled since you died", 3000);
        ...
        public void onVehicleDeath(Vehicle vehicle)
        {
            try
            {
                if (NAPI.Data.HasEntityData(vehicle, "loaderMats"))
                {
                    ...
                    player.ResetData("loadMatsTimer");
                    Trigger.ClientEvent(player, "hideLoader");
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Loading materials canceled, as the machine left Chekpot", 3000);
                }
            }
        ...
        private static void callback_armyclothes(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "change":
                    ...
                    {
                        Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't change clothes / shared", 3000);
                        return;
                    }
                    if (!client.GetData<bool>("ON_DUTY"))
                    {
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"You changed in the service form", 3000);
                        Manager.setSkin(client, 14, Main.Players[client].FractionLVL);
                        client.SetData("ON_DUTY", true);
                    }
                    else
                    {
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"You changed in casual clothes", 3000);
        ...
        private static void callback_armycombat(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            {
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not serve in the army", 3000);
                return;
            }
            if (!client.GetData<bool>("ON_DUTY"))
            {
