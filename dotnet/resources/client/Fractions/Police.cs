    ...
    class Police : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~g~Open warehouse"), new Vector3(policeCheckpoints[8].X, policeCheckpoints[8].Y, policeCheckpoints[8].Z + 0.7), 5F, 0.3F, 0, new Color(255, 255, 255));
        ...
        public static void callPolice(Player player, string reason)
        {
            NAPI.Task.Run(() =>
            {
                try {
                    ...
                    colshape.OnEntityExitColShape += (s, e) =>
                    {
                        if (e == player)
                        {
                            try
                            {
                                ...
                                Manager.sendFractionMessage(7, $"{e.Name.Replace('_', ' ')} canceled call");
                    ...
                    Manager.sendFractionMessage(7, $"Received a call from the player ({player.Value}) - {reason}");
                    Manager.sendFractionMessage(7, $"~b~Received a call from the player ({player.Value}) - {reason}", true);
        ...
        public static void clearWantedLvl(Player sender, params object[] arguments)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight, $"Passports with this number does not exist", 3000);
                        return;
                    }
                ...
                catch
                {
                    ...
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight, $"The player is not found", 3000);
                        return;
                    }
                ...
                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomRight, $"You shot wanted with passport owner {target}", 3000);
        ...
        public static void checkNumber(Player sender, params object[] arguments)
        {
            try
            {
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"No car vehicles found", 3000);
                    return;
                }
        ...
        public static void checkPerson(Player sender, params object[] arguments)
        {
            try
            {
                ...
                try
                {
                    ...
                    var gender = (acc.Gender) ? "Male" : "Female";
                    var lic = "";
                ...
                catch
                {
                    ...
                    foreach (DataRow Row in result.Rows)
                    {
                        ...
                        var gender = (genderBool) ? "Male" : "Female";
                        ...
                        Trigger.ClientEvent(sender, "executePersonInfo", $"{firstName}", $"{lastName}", $"{uuid}",$"{gender}", $"{wantedLvl}", $"{lic}", "License for weapons", "Driver's license");
                    }
                }
            }
            ...
            try
            {
                    ...
                    (sender.VehicleSeat == 0 || sender.VehicleSeat == 1))
                {
                    ...
                    if (Main.Players[sender].FractionID == 7 || Main.Players[sender].FractionID == 9 || Main.Players[sender].FractionID == 18)
                    {
                        ...
                        Commands.RPChat("me", sender, "Turn on the on-board computer");
                    }
                }
                return;
            }
        ...
        public static void interactPressed(Player player, int interact)
        {
            ...
            switch (interact)
            {
                ...
                case 59:
                    ...
                    GUI.Dashboard.OpenOut(player, Stocks.fracStocks[7].Weapons, "Armory", 6);
                    return;
                case 66:
                    if (Main.Players[player].FractionID != 7)
                    {
        ...
        public static void onPlayerDisconnectedhandler(Player player, DisconnectionType type, string reason)
        {
            try
            {
                ...
                if (player.HasData("CALLPOLICE_BLIP"))
                {
                    ...
                    Manager.sendFractionMessage(7, $"{player.Name.Replace('_', ' ')} canceled call");
