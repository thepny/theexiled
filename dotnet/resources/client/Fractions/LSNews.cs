    ...
    class LSNews : Script
    {
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                case 80:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You are not in News", 3000);
                        return;
                    }
        ...
        public void AdvertSend(Player player, int ID, string answer)
        {
            try {
                ...
                {
                    player.SendChatMessage("This announcement is more not available to change..");
                    Remove(ID, player);
                }
            }
            catch {
        ...
        public static void AddAdvert(Player player, string question, int price)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You filed an ad.Expect moderation", 3000);
                ...
                foreach (Player p in Main.Players.Keys.ToList())
                {
                    ...
                    if (Main.Players[p].AdminLVL >= 1) p.SendChatMessage($"~r~Announcement OT. {player.Name.Replace('_', ' ')} (ID {player.Value} | Ad №{id}): {question}");
                }
            }
            catch (Exception e)
            {
        ...
        public static void AddAnswer(Player player, int repID, string response, bool deleted = false)
        {
            try
            {
                ...
                if (!Adverts.ContainsKey(repID)) {
                    if(deleted) player.SendChatMessage("Announcements with a similar number was not found..");
                    return;
                }
                ...
                if(!deleted) {
                    try
                    {
                        ...
                        if(Adverts[repID].AuthorSIM >= 1) NAPI.Chat.SendChatMessageToAll("!{#00BCD4}" + $"Announcement OT. {Adverts[repID].Author.Replace('_', ' ')}: {response} | Telephone: {Adverts[repID].AuthorSIM}");
                        else NAPI.Chat.SendChatMessageToAll("!{#00BCD4}" + $"Announcement OT. {Adverts[repID].Author.Replace('_', ' ')}: {response}");
                        NAPI.Chat.SendChatMessageToAll("!{#00BCD4}" + $"Announcement editor: {player.Name.Replace('_', ' ')}.");
                    } catch {
                    }
                } else {
                    if(Main.Players[player].AdminLVL != 0) GameLog.Admin($"{player.Name}", $"delAd", $"{Adverts[repID].Author}");
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You deleted the player's announcement {Adverts[repID].Author}", 3000);
                    Player target = NAPI.Player.GetPlayerFromName(Adverts[repID].Author);
                    if (target != null) Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, $"{player.Name} deleted your announcement due to: {response}.", 3000);
                    response += " | Удалено";
                }
