...
namespace NeptuneEvo.Jobs
{
    class AutoMechanic : Script
    {
        ...
        public static void mechanicRepair(Player player, Player target, int price)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work by an auto mechanic", 3000);
                return;
            }
            if (!player.IsInVehicle || !player.Vehicle.HasData("TYPE") || player.Vehicle.GetData<string>("TYPE") != "MECHANIC")
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be in working transport", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can set the price of $ 50 to $ 300", 3000);
                return;
            }
            ...
            Trigger.ClientEvent(target, "openDialog", "REPAIR_CAR", $"Player ({player.Value}) suggested repairing your transport for ${price}");

            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered a player ({target.Value}) Repair transport for {price}$", 3000);
        }
        ...
        public static void mechanicRent(Player player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You rented work transport.Expect an order", 3000);
        ...
        public static void mechanicPay(Player player)
        {
            ...
            Notify.Send(driver, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) Paid repair", 3000);
            Commands.RPChat("me", driver, $"Receding the car");
        ...
        private static void order_onEntityExit(ColShape shape, Player player)
        {
            ...
            if (player.HasData("MECHANIC_DRIVER"))
            {
                ...
                Notify.Send(driver, NotifyType.Warning, NotifyPosition.BottomCenter, $"The client canceled the order", 3000);
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"You left the place of call of an auto mechanic", 3000);
                try
                {
                    NAPI.ColShape.DeleteColShape(orderCols[player]);
                    orderCols.Remove(player);
        ...
        public void Event_onPlayerEnterVehicleHandler(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                if (seatid == 0)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have a license category B", 3000);
                    ...
                    if (Main.Players[player].WorkID == 8)
                    {
                        if (NAPI.Data.GetEntityData(player, "WORK") == null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This working transport is already busy", 3000);
                                return;
                            }
                            if (Main.Players[player].Money >= mechanicRentCost)
                            {
                                Trigger.ClientEvent(player, "openDialog", "MECHANIC_RENT", $"Rent workers for ${mechanicRentCost}?");
                            }
                            else
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough " + (mechanicRentCost - Main.Players[player].Money) + "$ For rental of work", 3000);
                                VehicleManager.WarpPlayerOutOfVehicle(player);
                            }
                        }
                        else if (NAPI.Data.GetEntityData(player, "WORK") == vehicle) NAPI.Data.SetEntityData(player, "IN_WORK_CAR", true);
                        else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are already working", 3000);
                    }
                    else
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work as an auto mechanic.You can get a set in the city hall", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                    }
                }
            }
        ...
        public void Event_onPlayerExitVehicleHandler(Player player, Vehicle vehicle)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"If you do not sit in transport after 5 minutes, the working day will end", 3000);
        ...
        private void timer_playerExitWorkVehicle(Player player, Vehicle vehicle)
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have finished working day", 3000);
                        respawnCar(vehicle);
        ...
        public static void acceptMechanic(Player player, Player target)
        {
            if (Main.Players[player].WorkID == 8 && player.GetData<bool>("ON_WORK"))
            {
                ...
                {
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"Player ({player.Value}) Accepted your call.Stay at seats", 3000);
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player did not cause an auto mechanic", 3000);
            }
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work as an auto mechanic at the moment", 3000);
        }
        ...
        public static void cancelMechanic(Player player)
        {
            if (player.HasData("MECHANIC_CLIENT"))
            {
                ...
                player.ResetData("MECHANIC_CLIENT");
                Notify.Send(client, NotifyType.Warning, NotifyPosition.BottomCenter, $"Auto mechanic left the workplace, make a new order", 3000);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You canceled departure to the client", 3000);
                try
                {
                    NAPI.ColShape.DeleteColShape(orderCols[client]);
                    orderCols.Remove(client);
            ...
            if (NAPI.Data.GetEntityData(player, "IS_CALL_MECHANIC"))
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You canceled the challenge of the car mechanic", 3000);
                if (player.HasData("MECHANIC_DRIVER"))
                {
                    Player driver = player.GetData<Player>("MECHANIC_DRIVER");
                    driver.ResetData("MECHANIC_CLIENT");
            ...
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not cause an auto mechanic.", 3000);
        }
        ...
        public static void callMechanic(Player player)
        {
            if (!NAPI.Data.GetEntityData(player, "IS_CALL_MECHANIC"))
            {
                ...
                foreach (var p in players)
                {
                    ...
                    {
                        i++;
                        NAPI.Chat.SendChatMessageToPlayer(p, $"~g~[Dispatcher]: ~w~Player ({player.Value}) Called auto mechanic ~y~({player.Position.DistanceTo(p.Position)}м)~w~. Write ~y~/ma ~b~[ID]~w~, To accept the challenge");
                    }
                }
                if (i > 0)
                {
                    NAPI.Data.SetEntityData(player, "IS_CALL_MECHANIC", true);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Expect a call. In your area now {i} Auto mechanics.Use to cancel call /cmechanic", 3000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There are no car mechanics in your area.Try another time", 3000);
            }
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have already called an auto mechanic.Write to cancel /cmechanic", 3000);
        }
        ...
        public static void buyFuel(Player player, int fuel)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must work as an auto mechanic and be in the work machine", 3000);
                return;
            }
            if (player.GetData<int>("BIZ_ID") == -1 || BusinessManager.BizList[player.GetData<int>("BIZ_ID")].Type != 1)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be at refueling", 3000);
                return;
            }
            if (fuel <= 0)
            {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Belt with gasoline full", 3000);
                return;
            }
            if (!BusinessManager.takeProd(biz.ID, fuel, biz.Products[0].Name, biz.Products[0].Price * fuel))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Not enough Fuel at refueling", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You replenished the tank in your working machine to {player.Vehicle.GetSharedData<int>("FUELTANK")} liter", 3000);
        }
        ...
        public static void mechanicFuel(Player player, Player target, int fuel, int pricePerLitr)
        {
            ...
            if (fuel < 1)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't sell less liters", 3000);
                return;
            }
            if (pricePerLitr < 2 || pricePerLitr > 10)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can set the price of $ 2 to $ 10 per liter", 3000);
                return;
            }
            ...
            Trigger.ClientEvent(target, "openDialog", "FUEL_CAR", $"Player ({player.Value}) offered to fix your vehicles on {fuel} liter for ${fuel * pricePerLitr}");

            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered a player ({target.Value}) Make transport on {fuel} liter for {fuel * pricePerLitr}$.", 3000);
        }
        ...
        public static void mechanicPayFuel(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The mechanic not enough fuel to fill you", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You paid for repair of the vehicle refueling", 3000);
            Notify.Send(driver, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) Paid shipping transport", 3000);
            Commands.RPChat("me", driver, $"Required vehicle");
