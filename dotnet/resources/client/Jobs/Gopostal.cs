...
namespace NeptuneEvo.Jobs
{
    class Gopostal : Script
    {
        ...
        public static void GoPostal_onEntityEnterColShape(ColShape shape, Player player)
        {
            try
            {
                ...
                if (player.HasData("NEXTHOUSE") && player.HasData("HOUSEID") && NAPI.Data.GetEntityData(player, "NEXTHOUSE") == player.GetData<int>("HOUSEID"))
                {
                    ...
                    {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The owner is not at home, try later", 3000);
                        return;
                    }
                    ...
                    else
                    {
                        ...
                        NAPI.Player.PlayPlayerAnimation(player, -1, "anim@heists@narcotics@trash", "drop_side");
                        player.SetData("PACKAGES", 0);
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"You have no parcels left, take new", 3000);
                    }
                }
            }
        ...
        public static void getGoPostalCar(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work by courier", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You received Work transport", 3000);
