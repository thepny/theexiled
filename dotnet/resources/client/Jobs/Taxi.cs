...
namespace NeptuneEvo.Jobs
{
    class Taxi : Script
    {
        ...
        public static void taxiRent(Player player)
        {
            if (NAPI.Player.IsPlayerInAnyVehicle(player) && player.VehicleSeat == 0 && player.Vehicle.GetData<string>("TYPE") == "TAXI")
            {
                if (player.Vehicle.GetData<Player>("DRIVER") == null)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You rented a taxi. To offer the player to pay the passage, write /tprice [ID] [Price]", 3000);
        ...
        public static void taxiPay(Player player)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You paid the passage", 3000);
            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter, $"Player " + player.Name.Replace('_', ' ') + " Paid passage", 3000);
        }
        ...
        public void onPlayerEnterVehicleHandler(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                if (seatid == 0)
                {
                    ...
                    if (Main.Players[player].WorkID == 3)
                    {
                        if (NAPI.Data.GetEntityData(player, "WORK") == null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This taxi is already busy", 3000);
                                return;
                            }
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough " + (taxiRentCost - Main.Players[player].Money) + "$ on Rent taxi", 3000);
                                VehicleManager.WarpPlayerOutOfVehicle(player);
                            }
                        }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work in a taxi.You can get a set in the city hall", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                    }
                }
                else
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "If you want to send your route to the driver, then set the label on the map and click Z.", 5000);
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"In a taxi now there is no driver", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                    }
                }
            }
            ...
            try
            {
                ...
                if (Main.Players[player].WorkID == 3 && NAPI.Data.GetEntityData(player, "WORK") != null)
                {
                    ...
                    if (player.HasData("PASSAGER"))
                    {
                        ...
                        Notify.Send(passager, NotifyType.Warning, NotifyPosition.BottomCenter, $"Taxi driver left the workplace, make a new order", 3000);
                        NAPI.Task.Run(() => {
                            try
                            {
                ...
                try
                {
                    ...
                    if (NAPI.Data.GetEntityData(player, "CAR_EXIT_TIMER_COUNT") > 300)
                    {
                        ...
                        if (player.HasData("PASSAGER"))
                        {
                            ...
                            passager.ResetData("TAXI_DRIVER");
                            passager.SetData("IS_CALL_TAXI", false);
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"Taxi driver left the workplace, make a new order", 3000);
            ...
            if (Main.Players[player].WorkID == 3)
            {
                if (NAPI.Data.GetEntityData(player, "WORK") != null)
                {
                    ...
                    if (NAPI.Data.GetEntityData(vehicle, "TYPE") == "TAXI")
                    {
                        if (player == target)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can not set the price for the trip myself.", 3000);
                            return;
                        }

                        if (price > 200 || price < 20)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can not set the price above $ 200 or below $ 20", 3000);
                            return;
                        }
                        ...
                        Trigger.ClientEvent(target, "openDialog", "TAXI_PAY", $"Pay for passages ${price}?");
                        target.SetData("TAXI_SELLER", player);
                        target.SetData("TAXI_PAY", price);

                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered a player ({target.Value}) Pay for a trip {price}$", 3000);
                    }
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work at the moment", 3000);
            }
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work as a taxi driver", 3000);
        }
        ...
        public static void acceptTaxi(Player player, Player target)
        {
            if (Main.Players[player].WorkID == 3 && NAPI.Data.GetEntityData(player, "WORK") != null)
            {
                ...
                {
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"Taxi driver ({player.Value}) Accepted your call.Stay at seats", 3000);
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player did not cause a taxi or he was already accepted", 3000);
            }
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work as a taxi driver at the moment", 3000);
        }
        ...
        public static void cancelTaxi(Player player)
        {
            ...
            if (NAPI.Data.GetEntityData(player, "IS_CALL_TAXI"))
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You canceled a taxi call", 3000);
                if (player.HasData("TAXI_DRIVER"))
                {
                    Player driver = player.GetData<Player>("TAXI_DRIVER");
                    driver.ResetData("PASSAGER");
            ...
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not cause a taxi.", 3000);
        }
        ...
        public static void callTaxi(Player player)
        {
            if (!NAPI.Data.GetEntityData(player, "IS_CALL_TAXI"))
            {
                ...
                foreach (var p in players)
                {
                    ...
                    {
                        i++;
                        NAPI.Chat.SendChatMessageToPlayer(p, $"~g~[ДИСПЕТЧЕР]: ~w~Игрок ({player.Value}) caused a taxi ~y~({player.Position.DistanceTo(p.Position)}м)~w~. Write ~y~/ta ~b~[ID]~w~, To accept the challenge");
                    }
                }
                if (i > 0)
                {
                    NAPI.Data.SetEntityData(player, "IS_CALL_TAXI", true);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Expect the call acceptance.In your area now {i} taxi drivers. Use to cancel call /ctaxi", 3000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There are no taxi drivers in your area.Try another time", 3000);
            }
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have already called a taxi.Write to cancel /ctaxi", 3000);
        }
    }
}
