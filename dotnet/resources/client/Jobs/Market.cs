    ...
    class Market : Script
    {
        ...
        public void Event_MarketStart()
        {
            try
            {
                ...
                NAPI.Blip.CreateBlip(501, new Vector3(2367.3, 4881.5, 35.3), 1, 81, "Barrier crop", 255, 0, true, 0, 0); // Блип на карте
                NAPI.TextLabel.CreateTextLabel("~w~A corpse fellow", new Vector3(2367.39, 4881.526, 43.2), 10f, 0.2f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension); // Над головой у Ped
        ...
        {
            MarketItem(new FarmerProduct(7, 234, "Yield", true)),
            MarketItem(new FarmerProduct(2, 235, "Seeds", false)),
            MarketItem(new FarmerProduct(10, 14, "Strange grass", true)),
        };

        private static List<object> BuyItems = new List<object>()
        {
            MarketItem(new FarmerProduct(5, 235, "Seeds", false)),
        };
        #endregion
        ...
        public static void ButFarmerItem(Player player, int id, int count)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought {count} {name} for ${price}", 2000);
        ...
        public static void SellFarmerItem(Player player, int id, int count)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough the subject in inventory", 2000);
                return;
            }
            int price = 0;
            string name = null;
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You sold {count} {name} for ${price}", 2000);
