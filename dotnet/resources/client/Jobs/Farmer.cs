    ...
    public class Farmer : Script
    {
        ...
        public void Event_FarmerStart()
        {
            try
            {
                ...
                NAPI.Blip.CreateBlip(677, new Vector3(438.3, 6510.9, 22.4), 1, 24, "Farm", 255, 0, true, 0, 0); // Блип на карте
                NAPI.TextLabel.CreateTextLabel("~g~Fermer large", new Vector3(438.3554, 6510.949, 29.6), 10f, 0.1f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension); // Над головой у Ped
        ...
        #region  Начать или Finish work фермера
        ...
        public static void StartWork(Player player, bool state, string workname = null)
        {
            if (state)
            {
                ...
                if (item == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have no seed", 2000);
                    return;
                }
                for (int i = 0; i < Checkpoints.Count; i++)
                {
                    Trigger.ClientEvent(player, "createPlant", Convert.ToInt32($"10{i}"), "Plantation", 1, Checkpoints[i], 1, 0, 255, 0, 0);
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You got out to work farmer", 2000);
            }
            else
            {
                try
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You fired from the work of the farmer", 2000);
                }
                catch (Exception e)
                {
                    Log.Write(e.ToString(), nLog.Type.Error);
        ...
        private static void PlayerEnterCheckpoint(ColShape colShape, Player player)
        {
            try
            {
                ...
                if (!player.GetData<bool>($"seedplant{colID}")) 
                {
                    ...
                    NAPI.Task.Run(() => { Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Seeds are planted", 2000); }, 5000);
                }
                else if (player.GetData<bool>($"seedplant{colID}"))
                {
                    ...
                    if (exp == 100 && lvl < maxlvl)
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Congratulations on the new level of farmer: {lvl}", 2000);
                    }
                    else
                    {
                        if (lvl == maxlvl) exp = -1;
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Not enough places in inventory", 2000);
                        return;
                    }
                    ...
                    NAPI.Task.Run(() =>
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Vintage collected", 2000);
                    }, 5000);
                }
                ...
                Timers.Start($"{player.Name}farmer{colID}", 5000, () =>
                {
                    ...
                    if (player.GetData<int>($"regenplant{colID}") < 1) 
                    {
                        if (!player.GetData<bool>($"seedplant{colID}")) 
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Plantation is ready for landingи", 2000);
                            Trigger.ClientEvent(player, "createPlant", Convert.ToInt32($"10{colID}"), "Plantation", 1, Checkpoints[colID], 1, 0, 255, 0, 0);
                        ...
                        else
                        {
                            player.ResetData($"regenplant{colID}");
                            player.SetData($"seedplant{colID}", true); 
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Collect yield", 2000);
                            Trigger.ClientEvent(player, "createPlant", Convert.ToInt32($"10{colID}"), "Plantation", 1, Checkpoints[colID], 1, 0, 0, 255, 0);
                            NAPI.Task.Run(() => Timers.Stop($"{player.Name}farmer{colID}")); 
                        }
                    }
                });
        ...
        public static void LoadLvl(Player player, string workname)
        {
            try
            {
                ...
                if (result == null || result.Rows.Count == 0)
                {
                    ...
                    Log.Write($"I registered the player {player.Name}", nLog.Type.Warn);
                }
                else
                {
                    ...
                    Log.Write($"I downloaded the player {player.Name}", nLog.Type.Warn);
                }
        ...
        public static void SaveLvl(Player player, string workname)
        {
            try
            {
                ...
                if (result == null || result.Rows.Count == 0)
                {
                    ...
                    Log.Write("The user is entered in the database", nLog.Type.Warn);
                }
                else
                {
                    MySQL.Query($"UPDATE `{workname}` SET `level`={data[0]}, `exp`={data[1]}, `allpoints`={data[2]} WHERE uuid={acc.UUID}");
                    Log.Write($"I updated the player data {player.Name}", nLog.Type.Warn);
                }
            }
            catch (Exception e)
            {
