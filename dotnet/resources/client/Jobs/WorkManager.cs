    ...
    class WorkManager : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Work"), new Vector3(Points[0].X, Points[0].Y, Points[0].Z + 0.5), 10F, 0.3F, 0, new Color(255, 255, 255));
                ...
                NAPI.Blip.CreateBlip(408, new Vector3(105.4633, -1568.843, 28.60269), 1, 3, Main.StringToU16("post office"), 255, 0, true, 0, 0);
                NAPI.Blip.CreateBlip(198, new Vector3(903.3215, -191.7, 73.40494), 1, 46, Main.StringToU16("Charges"), 255, 0, true, 0, 0);
                NAPI.Blip.CreateBlip(198, new Vector3(1956.65015, 3769.12817, 31.0833454), 1, 46, Main.StringToU16("Charges"), 255, 0, true, 0, 0);//TODOTAXI
                NAPI.Blip.CreateBlip(198, new Vector3(1791.82837, 4586.595, 36.2361145), 1, 46, Main.StringToU16("Charges"), 255, 0, true, 0, 0);
                ...
                NAPI.Blip.CreateBlip(67, new Vector3(915.9069, -1265.255, 25.52912), 1, 25, Main.StringToU16("Collectors"), 255, 0, true, 0, 0);
                NAPI.Blip.CreateBlip(67, new Vector3(-1481.75537, -508.08847, 31.6868382), 1, 25, Main.StringToU16("Collectors"), 255, 0, true, 0, 0);
                NAPI.Blip.CreateBlip(67, new Vector3(-144.374817, 6354.90869, 30.3706112), 1, 25, Main.StringToU16("Collectors"), 255, 0, true, 0, 0);
                NAPI.Blip.CreateBlip(544, new Vector3(473.9508, -1275.597, 29.60513), 1, 40, Main.StringToU16("Motormechanics"), 255, 0, true, 0, 0);

                NAPI.Blip.CreateBlip(478, Truckers.getProduct[0], 1, 84, Main.StringToU16("Warehouse products"), 255, 0, true, 0, 0);
                NAPI.Blip.CreateBlip(478, Truckers.getProduct[1], 1, 36, Main.StringToU16("Composition of gasoline"), 255, 0, true, 0, 0);
                NAPI.Blip.CreateBlip(478, Truckers.getProduct[2], 1, 15, Main.StringToU16("Auto-component"), 255, 0, true, 0, 0);
        ...
        public static void Layoff(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must first finish the working day", 3000);
                return;
            }
            if (Main.Players[player].WorkID != 0)
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work by anyone", 3000);
        }
        public static void JobJoin(Player player, int job)
        {
            if (Main.Players[player].FractionID != 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can not get a job, because a member of the organization", 3000);
                return;
            }
            if (Main.Players[player].WorkID != 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"To start, dismiss from previous work.", 3000);
                return;
            }
            if (NAPI.Data.GetEntityData(player, "ON_WORK") == true)
            {
            ...
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Needed as minimum {JobsMinLVL[job]} level", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have a license category C", 3000);
                    return;
                }
                Main.Players[player].WorkID = job;
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You settled to work {JobList[job]}. Get to the start of work", 3000);
                ...
                case "start":
                    if (Main.Players[client].WorkID == 2)
                    {
                        if (!NAPI.Data.GetEntityData(client, "ON_WORK"))
                        {
                            ...
                            Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, $"You received 10 посылок, развезите их по домам", 3000);
                        ...
                        else Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You have already started working day", 3000);
                    }
                    else Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work the courier.You can get a set in the city hall", 3000);
                    return;
                case "get":
                    {
                        if (Main.Players[client].WorkID != 2)
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work by courier", 3000);
                            return;
                        }
                        if (!client.GetData<bool>("ON_WORK"))
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not start working day", 3000);
                            return;
                        }
                        if (client.GetData<int>("PACKAGES") != 0)
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not distribute all the parcels.You still have {client.GetData<int>("PACKAGES")} units", 3000);
                            return;
                        }
                        if (Houses.HouseManager.Houses.Count == 0) return;
                        client.SetData("PACKAGES", 10);
                        Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, $"You received 10 parcels.Divide them home", 3000);
                ...
                case "finish":
                    if (Main.Players[client].WorkID == 2)
                    {
                        if (NAPI.Data.GetEntityData(client, "ON_WORK"))
                        {
                            ...
                            Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, $"You have finished working day", 3000);
                        ...
                        else Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work", 3000);

                    }
                    else Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work by courier", 3000);
                    return;
                case "close":
                    MenuManager.Close(client);
                    return;
                ...
                case "button":
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"This order has already taken someone else", 3000);
                                return;
                            }
                            ...
                            Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, $"You took an order for delivery {order.Name} on {BusinessManager.BusinessTypeNames[biz.Type]}. Snachka purchase a load", 3000);
