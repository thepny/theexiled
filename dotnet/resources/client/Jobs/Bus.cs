...
namespace NeptuneEvo.Jobs
{
    class Bus : Script
    {
        ...
        {
            "ЛСПД  - Central square - ФБР",
            "Western ghetto - The airport -  Central square",
            "Western ghetto - Driving school - Central square",
            "Western ghetto  - The airport - Central square - Lawn mower - Power station - Taxusopark",
            "Western ghetto - Trucker - Eastern ghetto -  Incarcerate -  Mechanic",
            "Central square - Chumash - Army - Paleto Bay - Sandy Shorts",
        };
        ...
        private static Dictionary<string, Vector3> BusStations = new Dictionary<string, Vector3>()
        {
            ...
            { "Hotel at the airport", new Vector3(-888.1733, -2186.11, 9.900888) },
            { "The work of the lawn mower", new Vector3(-1354.111, -43.5153, 52.53339) },
            { "Power station", new Vector3(740.4898, 100.5469, 81.29053) },
            { "Taxusopark", new Vector3(918.1192, -188.8451, 74.84467) },
            { "Standard truckers", new Vector3(602.8403, -3018.18, 6.131153) },
            { "Eastern ghetto", new Vector3(837.6479, -1807.675, 29.10327) },
            { "Parking collectors", new Vector3(807.4769, -1195.802, 27.39124) },
            { "Parking Mechanics", new Vector3(449.7962, -1249.931, 30.22602) },
            { "Chu-so", new Vector3(-3104.435, 1097.097, 20.59407) },
            ...
            try
            {
                ...
                else
                {
                    ...
                    foreach (var p in Main.GetPlayersInRadiusOfPosition(player.Position, 30))
                        p.SendChatMessage("!{#3ADF00}After 10 seconds, the bus next to the route " + BusWaysNames[way]);
                }
            }
        ...
        private static void timer_busStop(Player player, int way, int check)
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You can go on", 3000);
                    var payment = Convert.ToInt32(BuswaysPayments[way] * Group.GroupPayAdd[Main.Accounts[player].VipLvl] * Main.oldconfig.PaydayMultiplier);
                    //NAPI.Data.SetEntityData(player, "PAYMENT", NAPI.Data.GetEntityData(player, "PAYMENT") + payment);
                    MoneySystem.Wallet.Change(player, payment);
                    GameLog.Money($"server", $"player({Main.Players[player].UUID})", payment, $"busCheck");
                    ...
                    foreach (var p in Main.GetPlayersInRadiusOfPosition(player.Position, 30))
                        p.SendChatMessage("!{#3ADF00}The bus goes on the route " + BusWaysNames[way]);
                }
                catch (Exception e)
                {
        ...
        public void onPlayerExitVehicleHandler(Player player, Vehicle vehicle)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"If you do not sit in transport in 60 seconds, the working day will end", 3000);
        ...
        public void onPlayerEnterVehicleHandler(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                if (player.VehicleSeat == 0)
                {
                    ...
                    if (Main.Players[player].WorkID == 4)
                    {
                        if (vehicle.GetData<Vehicle>("DRIVER") == null)
                        {
                            if (player.GetData<Player>("WORK") == null)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough " + (BusRentCost - Main.Players[player].Money) + "$ For rent a bus", 3000);
                                    VehicleManager.WarpPlayerOutOfVehicle(player);
                                }
                            }
                            else
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work the bus driver.You can get a set in the city hall", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                    }
                }
                else
                {
                    if (NAPI.Data.GetEntityData(vehicle, "ON_WORK"))
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You paid for travel {price}$", 3000);
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough money to pay", 3000);
                            VehicleManager.WarpPlayerOutOfVehicle(player);
                        }
                    }
                    else
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no driver online", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                    }
                }
        ...
        public static void acceptBusRent(Player player)
        {
            if (NAPI.Player.IsPlayerInAnyVehicle(player) && player.VehicleSeat == 0 && player.Vehicle.GetData<string>("TYPE") == "BUS")
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You rented a bus.You were distributed on the route {BusWaysNames[way]}", 3000);
            }
            else
            {
