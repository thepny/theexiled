...
namespace NeptuneEvo.Jobs
{
    class DrugFarm : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                NAPI.Blip.CreateBlip(140, new Vector3(2220.438, 5614.343, 53.60628), 1, 41, Main.StringToU16("Narcotics"), 255, 0, true, 0, 0);
        ...
        public static void StartWorkDay(Player player)
        {
            ...
            else
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "You started working day", 3000);

                Manager.sendFractionMessage(6, "[Informant illegal farm] Attention!Some strange type settled..");
                Manager.sendFractionMessage(7, "[Informant illegal farm] Attention!Some strange type settled..");
                Manager.sendFractionMessage(9, "[Informant illegal farm] Attention!Some strange type settled..");
                Manager.sendFractionMessage(14, "[Informant illegal farm] Attention!Some strange type settled..");
            ...
            switch (id)
            {
                ...
                case 382:
                    try
                    {
                        ...
                        if (drugs == null)
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "You have no herb", 3000);
                            return;
                        }
                        ...
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"You sold {drugs.Count} weed for {payment}$", 3000);
                ...
                NAPI.Task.Run(() => {
                    try
                    {
                        if (player != null && Main.Players.ContainsKey(player))
                        {
                            ...
                            if (tryAdd == -1 || tryAdd > 0) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough places in inventory", 3000);                       
