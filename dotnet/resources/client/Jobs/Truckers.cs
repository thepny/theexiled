...
namespace NeptuneEvo.Jobs
{
    class Truckers : Script
    {
        ...
        public static void truckerRent(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You started working a trucker.Open orders /orders", 3000);
        ...
        public static void getOrderTrailer(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work trucker", 3000);
                return;
            }
            ...
            if (player.HasData("TRAILER"))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have already received products", 3000);
                return;
            }
            int uid = player.GetData<int>("ORDER");
            ...
            switch (prod)
            {
                ...
                case 1:
                    if (biz.Type != 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not take an order for gasoline", 3000);
                        return;
                    }
                ...
                case 3:
                    if (biz.Type != 6)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not take an order for weapons", 3000);
                        return;
                    }
            ...
            if (TruckersTrailers)
            {
                ...
                player.SendChatMessage("If suddenly you lose your trailer, write to chat /findtrailer");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You received Trailer with a commodity, go to him and see 'your order'", 3000);
            }
            else
            {
                player.SetData("TRAILER", true);

                player.SendChatMessage("The necessary product was downloaded to your transport.");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have downloaded the transport to the goods, sit in the car and see 'your order'", 3000);
            }
        }
            ...
            try
            {
                ...
                if (player.VehicleSeat == 0)
                {
                    ...
                    if (Main.Players[player].WorkID == 6)
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы не можете Get Started", 3000);
                            VehicleManager.WarpPlayerOutOfVehicle(player);
                            return;
                        }
                        */
                        ...
                        {
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"At the moment, this transport is not available for transportation.Use another car (without trailer).", 3000);
                            VehicleManager.WarpPlayerOutOfVehicle(player);
                            return;
                        }
                        ...
                        if (!NAPI.Data.GetEntityData(vehicle, "ON_WORK"))
                        {
                            ...
                            {
                                Trigger.ClientEvent(player, "openDialog", "TRUCKER_RENT", $"Do you really want to get started?");
                            }
                        ...
                        else
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"In the car there is a driver", 3000);
                                VehicleManager.WarpPlayerOutOfVehicle(player);
                            }
                            else
                            {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work trucker.You can get a set in the city hall", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                    }
                }
        ...
        public static void playerGotProducts(Player player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Bring a load to {BusinessManager.BusinessTypeNames[biz.Type]}", 3000);
        ...
        public static void onEntityEnterDropTrailer(ColShape shape, Player player)
        {
            try
            {
                ...
                if (TruckersTrailers)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"The trailer must be in closeness", 3000);
                        return;
                    }
                }
                ...
                player.ResetData("WayPointBiz");
                if (ow != null)
                    Notify.Send(ow, NotifyType.Warning, NotifyPosition.BottomCenter, $"Your order on {order.Name} was delivered", 3000);
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You delivered trailer", 3000);
                return;
            }
        ...
        public static void truckerOrders(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work at the moment", 3000);
                return;
            }
