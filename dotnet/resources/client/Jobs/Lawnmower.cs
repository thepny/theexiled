...
namespace NeptuneEvo.Jobs
{
    class Lawnmower : Script
    {
            ...
            try
            {
                ...
                if (Main.Players[player].WorkID == 5)
                {
                    if (!NAPI.Data.GetEntityData(vehicle, "ON_WORK"))
                    {
                        ...
                        {
                            Trigger.ClientEvent(player, "openDialog", "MOWER_RENT", $"Start a lawn mower?");
                        }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work a lawn mower.You can get a set in the city hall", 3000);
                    VehicleManager.WarpPlayerOutOfVehicle(player);
                }
        ...
        public static void mowerRent(Player player)
        {
            ...
            {
                var way = 0;
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You started the work of a lawn mower, follow the checkpoints", 3000);
