...
namespace NeptuneEvo.Jobs
{
    class Miner : Script
    {
        ...
        private static int JobsMinLVL = 2; // С какого уровня можно Get settled на работу
        ...
        public void Event_ResourceStart()
        {
            try
            {
                NAPI.Blip.CreateBlip(618, new Vector3(2947.133, 2747.014, 42.28965), 1.5f, 22, Main.StringToU16("Quarry"), 255, 0, true, 0, 0); // Блип на карте
        ...
        public static void Layoff(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must finish the work day first", 3000);
                return;
            }
        ...
        public static void JobJoin(Player player)
        {
            if (Main.Players[player].WorkID != 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Are you already working: {Jobs.WorkManager.JobStats[Main.Players[player].WorkID - 1]}", 3000);
                return;
            }
            if (NAPI.Data.GetEntityData(player, "ON_WORK") == true)
            {
            ...
            //Dashboard.sendStats(player);
            //Trigger.ClientEvent(player, "OpenMiner", checkpointPayment, Main.Players[player].LVL, Main.Players[player].WorkID, NAPI.Data.GetEntityData(player, "ON_WORK2"), checkpointPayment2, checkpointPayment3);
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You got a job: {Jobs.WorkManager.JobStats[JobWorkId - 1]}", 3000);
            var jobsid = Main.Players[player].WorkID;
            Trigger.ClientEvent(player, "secusejob", jobsid);
            return;
        }
        ...
        public static void Layoff2(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Turn in the stone before you finish the work day", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't work anymore", 3000);
            }
        }
        public static void JobJoin2(Player player, int job)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't work at this job.", 3000);
                return;
            }
            if (NAPI.Data.GetEntityData(player, "ON_WORK") == true)
            {
