...
namespace NeptuneEvo.Jobs
{
    class Collector : Script
    {
            ...
            try
            {
                ...
                if (Main.Players[player].WorkID == 7)
                {
                    ...
                    if (!NAPI.Data.GetEntityData(vehicle, "ON_WORK"))
                    {
                        ...
                        {
                            if (Main.Players[player].Money >= 100) Trigger.ClientEvent(player, "openDialog", "COLLECTOR_RENT", "You really want to start working as a collector and rent a transport for $100?");
                            else {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough " + (100 - Main.Players[player].Money) + "$ For rent a bus", 3000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not work as a collector.You can get a set in the city hall", 3000);
                    VehicleManager.WarpPlayerOutOfVehicle(player);
                }
            }
        ...
        public void onPlayerExitVehicleHandler(Player player, Vehicle vehicle)
        {
            try
            {
                ...
                NAPI.Data.GetEntityData(player, "WORK") == vehicle)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"If you do not sit in transport through 3 minutes, the working day will end", 3000);
        ...
        public static void rentCar(Player player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You started working as a collector.Divide money on ATM.", 3000);
        ...
        public static void CollectorTakeMoney(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You still have bags with money ({player.GetData<int>("COLLECTOR_BAGS")} units)", 3000);
                return;
            }
            else
            {
        ...
        public static void CollectorEnterATM(Player player, ColShape shape)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "ATM is still full.try later", 3000);
                    return;
                }
                ...
                if (player.GetData<int>("COLLECTOR_BAGS") == 0)
                {
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, "Return to the database to take new bags with money", 3000);
                    Trigger.ClientEvent(player, "deleteWorkBlip");
