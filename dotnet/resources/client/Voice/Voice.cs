...
namespace NeptuneEvo.Voice
{
    public class Voice : Script
    {
        ...
        public static void PlayerQuit(Player player, string reson)
        {
            try
            {
                ...
                if (playerPhoneMeta.Target != null)
                {
                    ...
                    Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{playerName} Completed the challenge", 3000);
        ...
        {
            player.SendChatMessage("You have successfully rebooted voice chat for yourself (v1).");
        ...
        {
            player.SendChatMessage("You have successfully rebooted voice chat for yourself (v2).");
        ...
        {
            player.SendChatMessage("You have successfully rebooted voice chat for yourself (v3).");
        ...
        public static void PhoneCallCommand(Player player, Player target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "It is impossible to get a mobile phone", 3000);
                    return;
                }
                if (target != null && Main.Players.ContainsKey(target))
                {
                    ...
                    if (playerPhoneMeta.Target != null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "At the moment you are already talking", 3000);
                        return;
                    }
                    ...
                    if (targetPhoneMeta.Target != null)
                    {
                        ...
                        Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{playerName} Tried to get through", 3000);
                        return;
                    }
                    ...
                    NAPI.Task.Run(() => {
                        try
                        {
                            ...
                            Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, $"{targetName} Does not respond", 3000);
                            Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{playerName} Completed the challenge", 3000);
                        }
                        catch { }
                    ...
                    Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{playerName} Rings you.Open the phone to accept / reject the call", 3000);
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, $"You call {targetName}", 3000);
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Subscriber outside the network zone", 3000);
                }
        ...
        public static void PhoneCallAcceptCommand(Player player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"At the moment, no one calls you", 3000);
                    return;
                }
                ...
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"{playerName} Accepted your call", 3000);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You took a challenge from {targetName}", 3000);
        ...
        public static void PhoneHCommand(Player player)
        {
            try
            {
                ...
                {
                    if (!player.HasData("IS_DYING") && !player.GetData<bool>("CUFFED")) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"At the moment you do not speak by phone", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Call completed", 3000);
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"{playerName} Completed the bell", 3000);
