...
mp.keys.bind(Keys.VK_P, false, function () { // Телепорт (P)
    ...
    if(!lastWaypointCoords){mp.game.graphics.notify(`Error: ~n~~h~~r~No record of the last waypoint'a.`); return;}
...
function findZ(mp, maxAttempts, delay, wpos, oldpos) {
    ...
    timeout = setTimeout(function getZ() {
        ...
        if (!wpos.z && attempts < 10){
            ...
            mp.game.graphics.notify(`Error: ~n~~h~~r~Failed to get a coordinate Z.`);
            mp.players.local.freezePosition(false);
            clearTimeout(timeout);
        } else { //if z found - tp to wpos
            mp.players.local.position = new mp.Vector3(wpos.x, wpos.y, wpos.z+2);
            mp.players.local.freezePosition(false);
            mp.events.callRemote('notifyCoords', 'Teleport Po Coordinates:', wpos.x, wpos.y, wpos.z+1);
            clearTimeout(timeout);
        }
    }, delay)
}
...
function findWP(mp){
    ...
    if (wpos.z != 20) {
        mp.players.local.position = new mp.Vector3(wpos.x, wpos.y, wpos.z+2);
        mp.events.callRemote('notifyCoords', 'Teleport by coordinates:', wpos.x, wpos.y, wpos.z+1);
        return;
    }
...
mp.events.add('syncWP', function (bX, bY, type) {
    if(!mp.game.invoke('0x1DD1F58F493F1DA5')) {
		...
		if(type == 0) mp.events.call('notify', 2, 9, "The passenger has given you information about your route!", 3000);
		else if(type == 1) mp.events.call('notify', 2, 9, "Man from the list of contacts of your phone handed you the mark of his location!", 3000);
	} else {
		if(type == 0) mp.events.call('notify', 4, 9, "The passenger tried to give you information about the route, but you already have another route.", 5000);
		else if(type == 1) mp.events.call('notify', 4, 9, "A person from the list of contacts of your phone tried to give you a label of his location, but you have already installed another label.", 5000);
	}
});
...
mp.events.add('render', () => {
    ...
    if (deathTimerOn) {
        ...
        if(secondsLeft)
        {
            ...
            mp.game.graphics.drawText(`Death left ${('0'+minutes).slice(-2)}:${('0'+seconds).slice(-2)}`, [0.5, 0.8], {
                font: 0,
...
mp.events.add("playerRuleTriggered", (rule, counter) => {
    if (rule === 'ping' && counter > 5) {
        mp.events.call('notify', 4, 2, "Your ping is too big.Go later", 5000);
        mp.events.callRemote("kickclient");
    }
