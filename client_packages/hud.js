...
mp.events.add('render', () => {
    if (global.INTERACTIONCHECK === true && !global.menuOpened) {
        ...
        mp.game.ui.addTextComponentSubstringPlayerName("~h~Click ~g~ ~INPUT_CONTEXT~ ~s~ for interaction.");
...
mp.keys.bind(Keys.VK_J, false, function () { // belt system (J key)
    ...
    if (localplayer.isInAnyVehicle(false)) {
        ...
        if (hudstatus.belt) {
            ...
            mp.game.graphics.notify('~r~You dismissed the seat belt');
        }
        else {
            localplayer.setConfigFlag(32, false);
            //mp.events.call('notify', 2, 2, "Вы пристегнули ремень безопасности", 2000);
            mp.game.graphics.notify('~g~You fastened the seat belt');
        }
...
mp.keys.bind(Keys.VK_6, false, function () { // 5 key - cruise mode on/off
    ...
    if (new Date().getTime() - cruiseLastPressed < 300) {
        mp.events.call('openInput', 'Cruise control', 'Specify speed in km / h', 3, 'setCruise');
    } else {
...
mp.events.add('setCruiseSpeed', function (speed) {
	...
	if(speed < curSpeed) {
		mp.events.call('notify', 4, 9, "You can not set the speed less than it is at the moment, reduce speed and try again.", 6000);
		return;
	}
