...
mp.events.add('tclk', (id) => {
    ...
    if (setted)
        mp.events.call('notify', 1, 9, "You have already installed this modification", 3000);
    else {
        lsc.execute(`show(${false});`);
        lsc.active = false;
        opened = false;
        ...
        var title = (lscPage === "paint_menu_one" || lscPage === "paint_menu_two" || lscPage === "paint_menu_three") ? "Вы действительно хотите Paint машину в данный цвет?" : "Вы действительно хотите установить данную модификацию?";
...
mp.events.add('tuningSeatsCheck', function () {
    ...
    for (var i = 0; i < 7; i++)
        if (veh.getPedInSeat(i) !== 0) {
            mp.events.call('notify', 4, 9, 'Ask for all passengers', 3000);
            return;
        }
