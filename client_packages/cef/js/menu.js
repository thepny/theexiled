let menu = {
    ...
    add: function(id, type, name, addit){
        ...
        if(type == 0) block.append('<div class="right">Settle</div>');
        else if(type == 1) block.append('<div class="right">Buy</div>');
		else if(type == 10) block.append('<div class="right">Sell</div>');
        else block.append('<div class="right">Take</div>');
...
function openShop(data){
    ...
    menu.self.children('h1').html("Score");
...
function openBlack(data){
    ...
    menu.self.children('h1').html("Black market");
    var json = JSON.parse(data);
    json.forEach(function (item, i, arr) {
        // name, additional
        menu.add(i, 1,item[0],item[1]);
...
function openFib(data){
    ...
    menu.self.children('h1').html("Issuing arms");
    var json = JSON.parse(data);
    json.forEach(function (item, i, arr) {
        // name, additional
        menu.add(i, 2, item);
...
function openArmy(data){
    ...
    menu.self.children('h1').html("Issuing clothes");
    var json = JSON.parse(data);
    json.forEach(function (item, i, arr) {
        // name, additional
        menu.add(i, 2, item);
...
function openGang(data) {
    ...
    menu.self.children('h1').html("Autotransport");
    var json = JSON.parse(data);
    json.forEach(function (item, i, arr) {
        // name, additional
        menu.add(i, 2, item);
...
function openMafia(data) {
    ...
    menu.self.children('h1').html("Shipping");
    var json = JSON.parse(data);
    json.forEach(function (item, i, arr) {
        // name, additional
        menu.add(i, 2, item);
