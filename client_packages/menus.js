...
const ClubAlcos = {
    ...
    11: ["«En una cáscara de limón»", "«To cranberry»", "«Russian standard»"],
...
mp.events.add("openAlco", (club, modief, isOwner, stock) => {
    ...
    if (isOwner) {
        ...
        alcoUI.AddItem(new UIMenuItem("Smash", "Scaffold the selected drink"));
        alcoUI.AddItem(new UIMenuItem("Set the price", "Set price modifier for all products (from 50% to 150%)"));
    }
    ...
    alcoUI.ItemSelect.on(item => {
        ...
        else if (item.Text == "Smash") {
            mp.events.callRemote('menu_alco', 2, selectedAlco);
        }
        else if (item.Text == "Set the price") {
            global.menuClose();
            alcoUI.Close();
...
mp.events.add('stockTake', (index) => {
    ...
    switch (index) {
        ...
        case 1: //healkit
            ...
            global.input.set("Take a first-aid kit", "Enter the number of first-aid kits", 10, "take_stock");
            global.input.open();
            break;
        case 2: //weed
...
mp.events.add('stockPut', (index) => {
    ...
    switch (index) {
        ...
        case 1: //healkit
            mp.events.callRemote('setStock', "medkits");
            global.input.set("Put the first aid kit", "Enter the number of first-aid kits", 10, "put_stock");
            global.input.open();
            break;
        case 2: //weed
            mp.events.callRemote('setStock', "drugs");
...
mp.events.add('mavrshop', () => {
    let data = [
        ["Money Waste Service", ""],
        ["Drill for hacking", "100000$"],
        ["Lock for castles", "600$"],
        ["Military laundry", "2500$"],
        ["Screeds for hand", "2500$"],
        ["Bag on the head", "2500$"],
        ["Down the search", "3000$"],
        ["Armor", "200000$"]
    ];
...
mp.events.add('matsL', (act) => { //load
    ...
    switch (act) {
        case 1:
            global.input.set("Download mats", "Enter the number of mats", 4, "loadmats");
            global.input.open();
            break;
        case 2:
            global.input.set("Download mats", "Enter the number of mats", 4, "loadmats");
            ...
            break;
        case 3:
            global.input.set("Download drugs", "Enter the number of drugs", 4, "loaddrugs");
            global.input.open();
            break;
        case 4:
            global.input.set("Download the aid kit", "Enter the number of first-aid kits", 4, "loadmedkits");
            global.input.open();
            break;
    }
});
mp.events.add('matsU', (act) => { //unload
    ...
    switch (act) {
        ...
        case 2:
            ...
            break;
        case 3:
            global.input.set("Unload Drugs", "Enter the number of drugs", 4, "unloaddrugs");
            global.input.open();
            break;
        case 4:
            global.input.set("Unload the aid kit", "Enter the number of first-aid kits", 4, "unloadmedkits");
            global.input.open();
            break;
    }
});
...
let autoColors = ["Black", "White", "Red", "Orange", "Yellow", "Green", "Lightblue", "Blue", "Purple"];
let autoModels = null;
...
colors["Lightblue"] = [0, 205, 255];
colors["Blue"] = [0, 0, 230];
colors["Purple"] = [190, 60, 165];
...
mp.events.add('openClothes', (price) => {
    ...
    clothesHats[gender].forEach(function (hat, indexs) {
        ...
        hat.Colors.forEach(function (color, indexc) {
            var cloth_name = "Variation №" + color;
			var real_name;
			var gxt_name;
...
mp.events.add('addreport', (id_, author_, quest_) => {
    ...
    mp.events.call('notify', 0, 2, "New report came!", 1500);
})
...
mp.events.add('addadvert', (id_, author_, quest_) => {
	try {
        ...
        mp.events.call('notify', 0, 2, "New announcement came!", 3000);
	} catch(e) {
	}
})
