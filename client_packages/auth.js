...
mp.events.add('signup', function (regData) {
    ...
    if (checkLgin(username) || username.length > 50) {
        mp.events.call('notify', 1, 9, 'Login does not match the format or too long!', 3000);
        return;
    }

    if (pass1 != pass2 || pass1.length < 3) {
        mp.events.call('notify', 1, 9, 'Error in entering a password!', 3000);
        return;
    }
...
mp.events.add('newChar', function (slot, name, lastname) {
    if (checkName(name) || !checkName2(name) || name.length > 25 || name.length <= 2) {
        mp.events.call('notify', 1, 9, 'Right name format: 3-25 characters and the first letter name of the title', 3000);
        return;
    }

    if (checkName(lastname) || !checkName2(lastname) || lastname.length > 25 || lastname.length <= 2) {
        mp.events.call('notify', 1, 9, 'Correct surname format: 3-25 characters and the first letter of the head of the title', 3000);
        return;
    }
...
mp.events.add('transferChar', function (slot, name, lastname, pass) {
    if (checkName(name)) {
        mp.events.call('notify', 1, 9, 'The name does not match the format or too long!', 3000);
        return;
    }

    if (checkName(lastname)) {
        mp.events.call('notify', 1, 9, 'Surname does not match the format or too long!', 3000);
        return;
    }
